...
- **P3.2a** — ✅ **DONE — shipped as PR #224.** `PolicyStore.Save()` main-file write was plain `os.WriteFile` + `os.Rename` (atomic via rename but not fsynced); switched to `atomicWriteFile` for full durability parity with node-group / bandwidth / file-profile stores. Mirror of P3.1 follow-up #2 (PR #221).
- **P3.2b** — ✅ **DONE — shipped as PR #225.** `policyStore.ReplaceAll` was not persisted by the cluster apply path, so DP nodes restarted with stale rules until the next heartbeat. Fixed caller-side via `policyStore.Save()` immediately after `policyStore.ReplaceAll(...)` in `applyConfigSnapshot`, respecting PolicyStore's caller-side-Save convention. Mirror of P3.1 follow-up #3 (PR #222) — different shape (caller-side rather than internal save).
- **P3.2c (deferred / non-blocking, NOT a correctness gap)** — **HA-layer sync version-guard optimization.** Surfaced as a P2 review comment on PR #225 and consciously declined for that PR. The HA standby loop (`ha.go:148, 238`) calls `applyConfigSnapshot(bundle.Config)` every 5 s without a version guard, so every persisting sub-store (`pacStore.Set`, `globalProfileStore.ReplaceAll`, `nodeGroupStore.ReplaceAll`, `bandwidthManager.ReplaceAll`, and now `policyStore.Save()` after P3.2b) performs an atomic write+fsync per heartbeat regardless of whether the leader's config changed. **This is a class issue across sub-stores, not a per-store durability defect** — every store's in-memory state is already correct after `ReplaceAll`; the wasteful writes are an I/O-cost concern at steady state only. The correct fix is at the HA layer: add `lastSyncVersion` to `HAState` and skip the entire `applyConfigSnapshot(bundle.Config)` call when `bundle.Config.Version` hasn't advanced — mirroring the DP heartbeat path's existing `c.lastVersion` guard (`controlplane.go:1245`). That solves the wasteful-write pattern for every sub-store at once. Separate PR; non-blocking; can ship at any time.

**Sequencing (now historical):**
- ✅ **P3.2a** shipped first (PR #224) — hardened `PolicyStore.Save()` durability via `atomicWriteFile`.
...
#### P4.3 — Extract `initObservability` ✅ DONE `[travel-ok]`
- **Status:** ✅ shipped. `initObservability` is now a thin shim that calls `resolveObservabilityStartupConfig(s.fc, *s.syslogAddr, *s.syslogFormat, *s.otlpEndpoint, *s.auditLog, *s.requestLogPath, *s.requestLogMaxMB)` and `loadObservability(cfg)`. GeoIP-style: the resolver owns CLI vs. FileConfig precedence (none of the six inputs were pre-resolved on `startupState`). Loader returns void — no carry to `startupState`. Pinned in `startup_slice_contract_test.go` (14th slice).
- **Files:** `observability_startup.go`, `observability_startup_config.go`, `observability_startup_test.go`, `main.go`, `startup_slice_contract_test.go`.
