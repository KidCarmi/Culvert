...
The most consequential finding, **SC-1**, is that **only `apiContentScan` (DPI pattern add/remove) calls `saveConfigVersion`** — every other scanner-config admin handler emits `auditEvent` but not `saveConfigVersion`. The handlers split into two categories with different appropriate treatments (see §10 SC-1 for the per-handler classification): **persistent-config mutations** that change operator-authored state on disk (YARA rule CRUD, YARA settings, scan exclusions, DPI bypass, threat-feed domain allowlist) are reasonable rollback-tier candidates; **operational/transient actions** (force feed sync, force YARA reload, cache evict/clear) are not. A rollback from v3→v2 therefore restores DPI patterns but leaves the persistent-config mutations in place. This is structurally similar to the catStore-vs-categoryGroups asymmetry called out in P6.1 §10 UC-4, just wider in scope.
...
|---|---|---|---|
| **SC-1** | Scanner-config admin handlers do not call `saveConfigVersion`. Verified by `grep "saveConfigVersion" ui_security.go` returning only `apiContentScan` add/remove (plus `apiSecurity` and `apiFileblock`, both outside the scanner family). The handlers split into two classification groups: <br/><br/>**Category A — persistent-config mutations (reasonable `saveConfigVersion` candidates).** These change operator-authored, on-disk state and are valid rollback-tier events:<br/>• `apiSecYARARules` POST/PUT/DELETE (`ui_security.go:691, :725, :746, :761, :773`) — writes/deletes a rule file under `yaraDir`.<br/>• `apiSecYARASettings` PUT (`:640`) — flips engine knobs (`yaraEngineEnabledVar`, `MaxInflight`, `TimeoutSecs`, `OnTimeout`, `OnSaturation`).<br/>• `apiSecScanExclusions` PUT (`:838`) — replaces `scan_exclusions.json` hash/host lists.<br/>• `apiContentScanBypass` PUT (`:882`) — replaces `dpiScanner.bypassHosts`.<br/>• `apiDomainAllowlist` PUT (`:534`) — replaces threat-feed domain allowlist (persisted via `saveToDisk`).<br/><br/>**Category B — operational/transient actions (probably should NOT call `saveConfigVersion`).** These do not change operator-authored config; snapshotting them captures a "we ran an action" event without rollback value:<br/>• `apiSecFeedsSync` POST (`:511`) — manual trigger to re-pull URLhaus/OpenPhish; the next scheduled sync would converge to the same content. The persisted threat-feed DB is the result, not the action.<br/>• `apiSecYARAReload` POST (`:566`) — manual trigger to re-read disk; the on-disk rule files are unchanged. The snapshot-worthy event is the prior `apiSecYARARules` call that wrote the file.<br/>• `apiScanCache` evict/clear (`:935, :968`) — already correctly skips `saveConfigVersion`; cache is transient memory-only state.<br/><br/>**Recommended scope of the follow-up:** Category A only (5 handlers). Fix is one `saveConfigVersion(sessionAdmin(r), "<action>")` line per handler — mechanical. **Group with P6.1 UC-4** in a single config-versioning-coverage follow-up PR; they share the same shape and the same audit-tier-vs-snapshot-tier asymmetry. Category B handlers are not part of the recommended follow-up. | Yes | P6.1 UC-4 (`apiURLCat` / `apiURLCatHost` lack `saveConfigVersion`) |
