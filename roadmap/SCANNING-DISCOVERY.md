...
The most consequential finding, **SC-1**, is that **only `apiContentScan` (DPI pattern add/remove) calls `saveConfigVersion`** — every other scanner-config admin handler (YARA rule CRUD, YARA settings, YARA reload, threat-feed sync, threat-feed domain allowlist, scan exclusions, DPI bypass hosts, cache evict/clear) emits `auditEvent` but **not** `saveConfigVersion`. A rollback from v3→v2 therefore restores DPI patterns but leaves every other scanner-config mutation in place. This is structurally identical to the catStore-vs-categoryGroups asymmetry called out in P6.1 §10 UC-4, just much wider in scope.
...
| `globalScanExclusions` | `scan_exclusions.json` | tmp + rename (in `security_scan.go` exclusions Save path) | ❌ no fsync (verify) |
| `globalThreatFeed` | JSON DB at `secCfg.ThreatFeedDB` | tmp + rename via internal `save()` | ❌ no fsync (verify) |
...
**Implication:** scanner-config changes via the config YAML (ClamAV address, threat-feed DB path, YARA directory, scan size cap) require a process restart. Mutation via the admin API is the only runtime path. See §10 SC-5.
...
Order invariant: `appLifecycleCancel` (order 40) precedes `scan-svc-shutdown` (order 60), so the threat-feed goroutine has exited before the sidecar server stops. Pinned by `runtime_shutdown_wiring_test.go`'s canonical-order tests.
...
|---|---|---|---|
| **SC-1** | Scanner-config admin handlers do not call `saveConfigVersion`. Verified by `grep "saveConfigVersion" ui_security.go` returning only `apiContentScan` add/remove and `apiSecurity` / `apiFileblock`. The eight non-snapshotted handlers: `apiSecFeedsSync` (`:511`), `apiDomainAllowlist` (`:534`), `apiSecYARAReload` (`:566`), `apiSecYARASettings` (`:640`), `apiSecYARARules` POST/PUT/DELETE (`:691`), `apiSecScanExclusions` (`:838`), `apiContentScanBypass` (`:882`). All emit `auditEvent` / `auditEventDiff` but the rollback tier is silent. Fix is one `saveConfigVersion(sessionAdmin(r), "<action>")` line per handler — mechanical. **Group with P6.1 UC-4** in a single config-versioning-coverage follow-up PR; they share the same shape and the same audit-tier-vs-snapshot-tier asymmetry. | Yes | P6.1 UC-4 (`apiURLCat` / `apiURLCatHost` lack `saveConfigVersion`) |
...
| **SC-4** | `dpiScanner.Save()` (`scanner.go:130–150`) and `globalYARA.WriteRule` use plain `os.WriteFile` to a tmp file + `os.Rename` — atomic-via-rename but **not fsynced**. Mirrors the resolved P3.2a (PR #224) and the still-pending P3.1#1 + P6.1 UC-1. The fix is to route both through `atomicWriteFile`. **Group with P6.1 UC-1** in a single durability-hardening follow-up covering catStore.Save + globalCategoryGroups.Save + dpiScanner.Save + YARA WriteRule + (verify) `globalScanExclusions.Save` + (verify) threat-feed `save()`. | Yes | P6.1 UC-1; P3.1#1 (PAC `Set`); P3.2a (`PolicyStore.Save`, PR #224) |
| **SC-5** | `applyHotReload(fc)` (`main.go:2205–2260`) does not touch any scanner state — no YARA reload, no ClamAV reconnect, no threat-feed resync, no DPI pattern reload, no exclusions reload. Operators changing scanner config via the YAML file must restart the process. This is an intentional pre-existing design (the admin API is the runtime mutation surface) but is worth recording explicitly because (a) it's surprising for operators arriving from the rate-limit / rewrite / upstream subsystems that *do* hot-reload, and (b) any future "hot-reload everything" effort would need to consider scanner state carefully — most scanner config mutations cost more than a process restart. No action recommended now. | Yes | None (intentional design; recording for visibility) |
...
- **SC-1** independent of SC-3/SC-4; fold into the same config-versioning-coverage follow-up as P6.1 UC-4. One PR closes both at once.
