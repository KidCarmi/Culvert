...
| AU-31 | **Durability was reported from configuration, not from evidence.** `LoadRevocations` treats `os.IsNotExist` as a legitimate first run and returns, while `Configured` is already true — so `revocationsAreDurable()` claimed a revocation would survive a restart even when the parent directory was missing, read-only or unwritable and the first save was guaranteed to fail. The cluster API, the `session_revocation` row and the metric all read GREEN until some operator's logout, hours later, became the first write to discover the path was bad. "The file is not there yet" and "this path works" are different claims and the health plane reports the second. | NEW-adjacent (pre-existing, surfaced by CHAOS-68's health plane) → **CLOSED in the same PR** (Codex P2: the absent-file branch PROVES the path with the real write path — `SaveRevocations` creates the file and drives the same success/failure observers — so a bad path degrades at BOOT instead of at first logout) | M | was: `internal/session/session.go` `LoadRevocations`; gate `TestChaos68_UnwritablePathIsNotReportedDurable` + control `TestChaos68_WritablePathIsStillDurable`; see §38 |
