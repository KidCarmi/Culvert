...
| OCSP-10 | **No CRL fallback, and a certificate with no AIA responder is accepted unchecked.** The admin panel was titled "OCSP / CRL Revocation" when this row was recorded (since renamed "OCSP Revocation", terminology review 2026-09-21 T-57); only OCSP exists. Unchanged by CHAOS-65. | NEW (recorded) | L/M | `internal/ocsp/ocsp.go` `checkResponders` (`len(responders) == 0` ⇒ pass); see §35 |
