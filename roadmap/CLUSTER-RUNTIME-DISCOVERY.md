...
**No active concurrent-corruption surface was found** beyond what P6.3 already flagged (CA-7: `cpTLSConfig.cfg.ClientCAs` race, unverified under `-race`). Every cluster-domain store guards its hot path with `sync.RWMutex` or atomic primitives; gRPC server stop uses `srv.GracefulStop()` to drain in-flight RPCs. **Six classes of pre-existing gaps are tracked separately** in §13: the P3.4 consolidated apply-then-persist gap above; the absence of `saveConfigVersion` from every cluster admin handler (parallel of P6.3 CA-1); the `runClusterUpdate` detached orchestrator goroutine spawned without a parent context; the absence of split-brain prevention in HA failover; the `ha_config.json` persistence path using best-effort `os.WriteFile` rather than `atomicWriteFile`; and `ClusterStore.Save()` using RLock instead of Lock (acknowledged in source as "worth re-evaluating in a follow-up").
...
| **CL-R-6**: `clusterRole` reads vs `enableControlPlane` write | `clusterRoleMu sync.RWMutex` | LOW (resolved) | Every read site takes RLock; every write takes Lock |
...
**No P5.3-style ownership refactor is warranted.** Every cluster store has a coherent lock contract; the gaps in §13 are about durability (P3.4), governance (`saveConfigVersion`), goroutine ownership (`runClusterUpdate`), persistence atomicity (`ha_config.json`), and operational properties (split-brain), not races.
...
**Resolution on rejoin:** when the partition heals, **the standby's most recent state wins** for `ClusterState` (because the original leader's RPC connection to the standby may be re-established; behavior on rejoin is **UNCERTAIN** without an integration test). Admin mutations on the partition-loser side between failover and rejoin are silently discarded.
...
| **CL-4** | **HA split-brain is structurally unmitigated.** No quorum, no lease, no fencing. Standby promotes after 15s of `HASync` failures; original leader continues running. Both sides accept admin mutations during partition; resolution on rejoin is **UNCERTAIN**. See §11.1. **This is a real operational risk** but the fix is non-trivial (introduce an external arbiter, or a DP-quorum-based fencing protocol). Out of P6.4 scope; flagged for a dedicated HA-hardening design discussion. | operator-visible — silent state divergence under network partition | None — first-time recording |
| **CL-5** | **`runClusterUpdate` does not coordinate with HA failover.** If a failover happens mid-update, the new leader's view of `cluster_update.json` may be stale (the file is local, not HA-replicated). The orchestrator may resume from a stale checkpoint or start fresh; **UNCERTAIN** without an integration test. See §11.6. Out of P6.4 scope; flagged for the same HA-hardening design discussion as CL-4. | operator-visible — rolling update behavior under failover | None |
...
**None of CL-1 through CL-10 are required for P6 to advance.** CL-1 is the P3.4 work surface (now unblocked); CL-2 / CL-3 / CL-7 are mechanical follow-ups that can be grouped with existing durability / observability PRs; CL-4 / CL-5 require dedicated HA-hardening design; CL-6 / CL-8 / CL-9 / CL-10 are governance / observability / record-only.
...
- **CL-7 (`ha_config.json` atomicity)** — group with the durability-hardening follow-up covering P6.1 UC-1 / P6.2 SC-4 / P6.3 CA-3 — none of those have shipped yet, so a single PR could cover all four.
- **CL-4 / CL-5 (HA split-brain + update coordination)** — dedicated HA-hardening design discussion; not a quick fix.
