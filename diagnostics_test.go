...
func TestApiDiagnostics_SyslogFeedNotConfigured(t *testing.T) {
	...
	t.Cleanup(func() { syslogConfiguredAddr = prevAddr; publishSyslogWriter(prevSW) })
...
// InitSyslog never connected (the active writer nil), so the SIEM feed is silently
...
// InitSyslog succeeded (the active writer non-nil), matching the production success
...
// first target (the active writer non-nil, syslogConfigured == that target), then a
...
// activeSyslog() != nil check reported OK; the feed to the intended collector is
// actually down.
