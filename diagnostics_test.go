...
// TestApiDiagnostics_SyslogFeedOK — a syslog target was configured and a
// writer is installed and aimed at it, matching the production success path.
// UDP construction never blocks on a handshake, so it stands in for a live
...
func TestApiDiagnostics_SyslogFeedOK(t *testing.T) {
	...
	if strings.Contains(found.Message, "delivering") {
		t.Errorf("a UDP feed must not claim delivery it cannot observe: %q", found.Message)
	}
}
