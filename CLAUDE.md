...
- **Startup slices (SAFE pilots + MEDIUM tranche + HIGH sweep — 21 shipped)**: extracted under the `<domain>_startup_config.go` (resolver + DTO) + `<domain>_startup.go` (loader) + `<domain>_startup_test.go` (per-slice tests) layout — the 11 SAFE pilots (`fileblock`, `inspection_rules`, `geoip`, `pac`, `mtls_ocsp`, `ui_extras`, `legacy_auth_providers`, `rewrite_default_action`, `ui_access_policy`, `metrics_token`, `session`), the MEDIUM tranche (`blocklist`, `connlimit`, `observability`, `auth`), plus `logstore`, `persistent_admin_state`, `upstream_pool`, `rootca`, `background_services`, `urlcategories`. Each `init*` in `main.go` is a thin shim; **env values are read in the shim and passed to the resolver as params** so resolvers stay pure. `startup_slice_contract_test.go` pins the resolver convention (purity + determinism + no-fc-mutation) — new slices append to its table. **Do not re-extract shipped slices.** `startDataPlane` is **deliberately NOT a slice** (its params arrive pre-resolved; it is runtime wiring, not config resolution — forcing the pattern would be ceremony). Remaining fat startup functions: `initScanning` (~97 ln), `initCluster` (~93), `initCDR` (~87) — the genuinely goroutine-heavy trio; map each in full before moving code, one per commit.
