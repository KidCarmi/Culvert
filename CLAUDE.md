...
- **Startup slices (SAFE pilots + MEDIUM tranche — shipped)**: 16 slices from `roadmap/ARCH_DISCOVERY.md` are extracted under the `<domain>_startup_config.go` (resolver + DTO) + `<domain>_startup.go` (loader) + `<domain>_startup_test.go` (per-slice tests) layout — the 11 SAFE pilots (`fileblock`, `inspection_rules`, `geoip`, `pac`, `mtls_ocsp`, `ui_extras`, `legacy_auth_providers`, `rewrite_default_action`, `ui_access_policy`, `metrics_token`, `session`) plus the MEDIUM tranche (`blocklist` P4.1, `connlimit` P4.2, `observability` P4.3, `auth` P4.4) plus `logstore`. Each `init*` in `main.go` is a thin shim; **env values are read in the shim and passed to the resolver as params** so resolvers stay pure. `startup_slice_contract_test.go` pins the resolver convention (purity + determinism + no-fc-mutation) — new slices append to its table. **Do not re-extract shipped slices.** Remaining fat startup functions are the HIGH-risk zone (`initScanning` ~97 ln, `initCluster` ~93, `initCDR` ~87, `initURLCategories` ~65, `initBackgroundServices`, `startDataPlane`, `initRootCA`, `initUpstreamPool`, `initPersistentAdminState`) — extract smallest-first, one per commit, mapping each before moving code.
