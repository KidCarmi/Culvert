...
- `CULVERT_LEGACY_GH_TAG_CHECK` — legacy **update-path** GitHub-tags fallback (`update.go`). Default = OFF: the update checker makes **no unauthenticated GitHub API call** (a private-repo build cannot 404-brick this path; the Docker-registry `/api/update/check` remains the live visibility path). Set to `true`/`1`/`yes`/`on` to re-enable the `checkGitHubLatestTag` fallback during the compat window (break-glass; a typo/unset stays OFF, fail-safe). Read once at startup; gate state surfaces in `/api/update/status` (`legacy_gh_tag_check`). Covers the runtime update path only — NOT install-time script fetches (`internal/bootstrap`, deferred to M2).
