...
func TestC1_RouteMetadata_Locked141(t *testing.T) {
	const want = 233 // 222 (incl. the 3 ADR-0027 LDAP IdP routes) + 6 /api/policy-learning/* (ADR-0025 M5A) + 1 /api/backups (backup-archive visibility) + 3 FrontendV2 preview routes (FE-1B: /app, /app/, /assets/) + 1 /api/authpolicy/killswitch (Auth Exempt runtime kill-switch break-glass control)
