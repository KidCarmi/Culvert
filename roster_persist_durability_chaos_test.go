...
// FE-6A.0 / FE-6A.2 (record 6ARR, merge 8): on this tree the users and
// change-password handlers are the FENCED, persist-before-publish shapes —
// POST /api/auth/users is create-only, an existing account is changed by PUT,
// every mutation echoes the roster document revision (fencedUsersPath) or the
// caller's security generation, refusals are the typed JSON dialect
// (500 persist_failed, nothing changed) and a delete answers 200 with a body.
// The requests below carry those fences so each gate reaches the durable write
// it exists to test instead of being refused at the fence (428) — the
// assertions are the original ones.
func TestChaos70_DeleteUser_PersistFailure_DoesNotClaimSuccess(t *testing.T) {
	...
	apiAuthUsers(w, jsonReq(http.MethodDelete, fencedUsersPath("username=doomed"), nil))
...
func TestChaos70_SetUser_PersistFailure_DoesNotClaimSuccess(t *testing.T) {
	...
	w := httptest.NewRecorder()
	apiAuthUsers(w, jsonReq(http.MethodPut, fencedUsersPath(), map[string]any{
		"username": "doomed", "role": string(RoleViewer),
	}))

	if w.Code >= 200 && w.Code < 300 {
		t.Errorf("PUT must not report success when the role change could not be durably persisted; got %d body %s", w.Code, w.Body.String())
	}
...
func TestChaos70_ChangePassword_PersistFailure_DoesNotClaimSuccess(t *testing.T) {
	...
	gen, known := cfg.UserSecurityGeneration("doomed")
	if !known {
		t.Fatal("seeded account has no security generation")
	}
...
func TestChaos70_Control_HealthyDiskStillApplies(t *testing.T) {
	...
	apiAuthUsers(w2, jsonReq(http.MethodDelete, fencedUsersPath("username=doomed"), nil))
	// FE-6A.0: a delete answers 200 with {ok, revision, sessionsRevoked, …},
	// not 204 — the caller needs the new revision to fence its next write.
...
func TestChaos70_Control_MutationErrorIsNotMaskedAsPersistFailure(t *testing.T) {
	...
	w := httptest.NewRecorder()
	apiAuthUsers(w, jsonReq(http.MethodDelete, fencedUsersPath("username=only"), nil))
...
func TestChaos70_Wall_NoRosterPersistErrorIsDiscarded(t *testing.T) {
	...
	rosterPersistCalls := map[string]bool{
		...
		"SetAuthDurably":         true,
		// FE-6A.0 / FE-6A.2 (record 6ARR, merge 8): the administrative
		// mutations persist through the roster transaction (commitRoster —
		// persist-before-publish, nothing changes in memory until the write
		// landed) behind these four store methods, so THEY are the roster-
		// persisting calls this file makes for create / update / delete /
		// self-service password change.
		"CreateUIUser":         true,
		"UpdateUIUser":         true,
		"DeleteUIUserFenced":   true,
		"ChangeUIUserPassword": true,
	}
	...
	// Not-vacuous: on this tree ui_auth.go carries the four FE-6A.0 transaction
	// calls (CreateUIUser / UpdateUIUser / DeleteUIUserFenced /
	// ChangeUIUserPassword — the administrative mutations), two
	// mutateRosterBestEffort (login path) and one SetAuthDurably (first-time
	// setup) — no raw SaveUIUsersFile remains. A
