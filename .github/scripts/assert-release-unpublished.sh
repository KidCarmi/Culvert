...
#
# SCOPE: this guard is about the GitHub Release only. It does NOT license any
# change to a public GHCR tag — a registry tag is public the instant it is
# written, so Draft is not a visibility boundary for it, and an earlier revision
# that let a Draft unlock repointing an exact version tag was wrong. Retry
# safety for the registry comes from resolve-release-candidate.sh instead, which
# binds each version to one digest so a retry promotes the same bytes.
#
