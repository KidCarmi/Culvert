...
func (s *caLoadRecoveryState) snapshotLocked() caLoadRecoverySnapshot {
	return caLoadRecoverySnapshot{
		...
		LoadFailureClass: sslInspectionLoadFailureClass(),
		Attempts:         s.attempts,
...
func noteSSLInspectionRecoveredWith(how string, also func(*caLoadRecoveryState)) {
	...
	// snapshot reads the latch under the same lock. The FE-6B.0 bounded
	// failure CLASS latch (loadFailureClass) is cleared in the same transition.
...
func tryInspectionCARecovery(cfg rootCAStartupConfig, attempt int) (attempted bool, err error) {
	...
	// A deferral is an attempt that RAN and did not proceed: it is recorded
	// as one transition too (count + bounded class), exactly like a failed
	// attempt, so the record never pairs a count with another attempt's
	// outcome (PR #1440's invariant applies to every exit of this function).
	writes := cfg.Path == "" || certMgr.Ready()
	if writes {
		if serr := settleCertTarget(certOpsStore(), "root_ca", "writer", certWriterCARecovery); serr != nil {
			derr := fmt.Errorf("recovery deferred: %w", serr)
			caLoadRecovery.update(func(s *caLoadRecoveryState) {
				s.attempts++
				s.lastErr = caFaultClass(derr)
			})
			return true, derr
		}
	}
	...
	// hand?" branch above returns before it. The recorded error is the FE-6B.0
	// bounded class (the admin surface publishes it as loadRecoveryClass; the
	// path and the OS text stay off it).
	if err := attemptInspectionCARecovery(cfg); err != nil {
		caLoadRecovery.update(func(s *caLoadRecoveryState) {
			s.attempts++
			s.lastErr = caFaultClass(err)
		})
		return true, err
	}
...
func runInspectionCARecoveryLoop(ctx context.Context, cfg rootCAStartupConfig, sched caRetrySchedule) {
	...
	caLoadRecovery.update(func(s *caLoadRecoveryState) { s.gaveUp = true })
	if logger != nil {
...
type caLoadRecoverySnapshot struct {
	LoadFailure string
	// LoadFailureClass is the FE-6B.0 bounded class of LoadFailure, read in the
	// SAME locked snapshot so a status surface that publishes the class (never
	// the raw reason) still pairs the latch with the record it was written with.
	LoadFailureClass string
	Attempts         int64
	Recovered        bool
	GaveUp           bool
	LastErr          string
...
func caLoadRecoveryStatus() caLoadRecoverySnapshot {
	...
	return caLoadRecovery.snapshotLocked()
}
...
// setCALoadRecoveryObserverForTest installs (or, with nil, removes) the
// transition observer under the record's own lock, so the swap is ordered
