...
- Destination-category rules no longer allocate once per rule per request.
  ...
  directly. On a 4-core box, with both arms benchmarked in one session
  (medians of n=5), the probe goes 204.4 → 137.4 ns on a miss and 170.1 →
  105.7 ns on a hit, at 1 alloc → 0 in every posture; through the real policy
