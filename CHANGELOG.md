...
- **The tracing-header bound is now proven over a socket, and one of its stated
  ...
  and the claim that they did (in the code comment, this changelog and the
  operator runbook) came from measuring the function rather than the wire; all
  three are corrected. The bound is **not** redundant for it: it is load-bearing
  on 130 delivered byte values — `TAB`, `SPACE` and every byte `0x80`–`0xFF` —
  and the length half is reachable in full. Seven new gates drive the real
  `setupRequestTracing` behind a real listener and pin the delivered/refused
  partition in both directions, so a future Go release that widens what a header
  value may carry fails the build instead of quietly widening what reaches the
  forensic log. No behaviour change: bound, charset, counters and emitted bytes
  are untouched.
