...
var configSurfaces = []configSurfaceRow{
	...
	{ID: "idp_profiles", Kind: kindConfig, Owner: "idpRegistry", Sensitive: true,
		ClusterSynced: true, SnapshotCap: maxSnapIdPProfiles, WireWipeCapable: true,
		Note:     "carries OIDC client secrets and LDAP bind credentials by design (DP-local auth); redacted for unenrolled callers; compile-validated ReplaceAll, rejection aborts extended state; ConfigSnapshot field has NO omitempty (FE-6A.0 R5): an EMPTY registry propagates CP→DP as an explicit [] so the last delete clears the DP copy — nil (an older CP) still means skip",
...
var offRegistryRollbackExclusions = []rollbackExcludedSetting{
	...
	{ID: "inspection_root_ca", Reason: "the SSL-inspection root CA and its rotation are forward-only trust decisions; rollback would restore a superseded CA",
		sources: []string{"ui_certificates.go:apiCARotate"}},
	{ID: "custom_tls_certificates", Reason: "uploaded admin-UI and MITM certificates are forward-only trust mutations; rollback would restore superseded certificates",
		sources: []string{"ui_certificates.go:apiCertsUpload"}},
	{ID: "ocsp_revocation_checking", Reason: "relaxing revocation checking via rollback would silently re-permit certificates the admin tightened against",
		sources: []string{"ui_certificates.go:apiOCSPConfig"}},
