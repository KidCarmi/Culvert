...
type d16bTestRig struct {
	...
	// Knobs for tests to control what the fake exec returns.
	failOnArgvContains atomic.Pointer[string] // when set, exec fails iff argv contains this substring
	healthFail         atomic.Bool            // when set, health probe returns ReadyOK=false
}
...
func startD16bRig(t *testing.T) *d16bTestRig {
	...
	r.SetExecHooksForTest(
		...
		func(cmd *exec.Cmd) error {
			if marker := rig.failOnArgvContains.Load(); marker != nil && *marker != "" {
				for _, a := range cmd.Args {
					if strings.Contains(a, *marker) {
						return &exec.ExitError{ProcessState: nil} // exit 1 (close enough)
					}
				}
			}
			return nil
