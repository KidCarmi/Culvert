...
func TestSetUIUser_RoleChangePreservesTOTP(t *testing.T) {
	...
	// FE-6A.0 R12 (record 6ARR, merge 7): demoting the LAST admin is refused
	// at the mutation itself (errRosterLastAdmin), so a second admin is seeded
	// first. This gate is about the second factor surviving a role change,
	// not about that guard, which has its own rows.
	if err := c.SetUIUser("bob-peer-admin", "Passw0rd1", RoleAdmin); err != nil {
		t.Fatalf("seed peer admin: %v", err)
	}
...
func TestAPIChangePassword_PreservesTOTP(t *testing.T) {
	...
	// FE-6A.2 (record 6ARR, merge 7): every administrative roster mutation
	// requires a persistence path (503 persistence_not_configured otherwise)
	// and the self-service password change is fenced on the caller's own
	// security generation (428 without it). Both are satisfied here so the
	// gate reaches the credential write it exists to test.
	cfg.mu.Lock()
	...
	gen, known := cfg.UserSecurityGeneration("grace")
	if !known {
		t.Fatal("seeded user has no security generation")
	}
...
func TestAPIAuthUsers_AdminPasswordSetPreservesTOTP(t *testing.T) {
	...
	// FE-6A.0/6A.2 (record 6ARR, merge 7): the roster needs a persistence
	// path (503 persistence_not_configured otherwise); POST /api/auth/users is
	// CREATE-only on this contract (409 user_exists — "update it instead"), so
	// an admin setting an EXISTING account's password is PUT, fenced on the
	// roster document revision (fencedUsersPath echoes the current one). The
	// request below therefore reaches the credential write this gate exists
	// to test.
	cfg.mu.Lock()
	cfg.uiUsers = map[string]*uiAdminUser{}
	cfg.uiUsersFile = filepath.Join(t.TempDir(), "ui_users.json")
	...
	ctx := context.WithValue(context.Background(), uiRoleKey{}, RoleAdmin)
	req := httptest.NewRequestWithContext(ctx, http.MethodPut, fencedUsersPath(), bytes.NewReader(body))
	rec := httptest.NewRecorder()

	apiAuthUsers(rec, req)
	if rec.Code != http.StatusOK {
		t.Fatalf("PUT /api/auth/users returned %d: %s", rec.Code, strings.TrimSpace(rec.Body.String()))
	}
	assertTOTPIntact(t, cfg, "heidi", "PUT /api/auth/users password set")
}
