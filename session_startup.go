...
func loadSession(cfg sessionStartupConfig) error {
	...
	if cfg.RevocationsFile != "" {
		if err := loadPersistedRevocations(cfg.RevocationsFile); err != nil {
			return err
		}
	}
...
// loadPersistedRevocations attaches the revocations file and loads it.
//
// Extracted from loadSession so the corrupt/quarantine branch is not four
// levels deep (nestif); the nesting is otherwise unchanged, so each comment
// below still sits on exactly the branch it describes.
func loadPersistedRevocations(path string) error {
	...
	noteResidualQuarantine("session_revocations", path)
	if err := sessionRevoked.LoadRevocations(); err != nil {
		...
		if revocationLoadIsCorrupt(err) {
			if quarantineCorruptStateFile("session_revocations", path, err) == "" {
