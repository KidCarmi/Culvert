...
func TestSrcPrefix_DifferentialRandomized(t *testing.T) {
	...
	for i := 0; i < 400; i++ {
		...
		if i%2 == 0 {
			...
			ones := rng.Intn(33)
			sourceIP = fmt.Sprintf("%d.%d.%d.%d/%d", b[0], b[1], b[2], b[3], ones)
			for j := 0; j < 6; j++ {
				c := [4]byte{byte(rng.Intn(256)), byte(rng.Intn(256)), byte(rng.Intn(256)), byte(rng.Intn(256))}
				probes = append(probes, fmt.Sprintf("%d.%d.%d.%d", c[0], c[1], c[2], c[3]))
			}
			// Probes derived FROM the network itself: random uniform addresses
			// almost never land inside a long prefix, so without these the
			// randomized half would only ever exercise the miss branch.
			...
			ones := rng.Intn(129)
			sourceIP = fmt.Sprintf("%s/%d", net.IP(b[:]).String(), ones)
			for j := 0; j < 6; j++ {
				var c [16]byte
				for k := range c {
					c[k] = byte(rng.Intn(256))
				}
				probes = append(probes, net.IP(c[:]).String())
			}
			if _, ipNet, err := net.ParseCIDR(sourceIP); err == nil {
				probes = append(probes, ipNet.IP.String())
				flipped := append(net.IP(nil), ipNet.IP...)
				flipped[15] ^= 0x01
				probes = append(probes, flipped.String())
			}
		}
