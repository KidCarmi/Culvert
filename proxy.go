...
func handleTunnelInspect(w http.ResponseWriter, r *http.Request, tlsSkipVerify bool, match *PolicyMatch) {
	...
	for {
		...
		logger.Printf("SSL_INNER %s %s %s%s (profile=%q filtering=%v)", clientIP, req.Method, sanitizeLog(hostOnly), sanitizeLog(req.URL.Path), sanitizeLog(profileName), hasProfile)
