...
func TestMaybeFailOpenClient_LearnOnlyPinningUnderFailOpen(t *testing.T) {
	...
	if _, ok := autoExclude().Contains(scope, scopeGen(t, "fo"), "pinned.example"); !ok {
	...
	if _, ok := autoExclude().Contains(scope, scopeGen(t, "fo"), "noise.example"); ok {
	...
	if _, ok := autoExclude().Contains(fcScope, scopeGen(t, "fc"), "pinned.example"); ok {
