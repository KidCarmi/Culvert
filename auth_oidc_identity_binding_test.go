...
func TestOIDCAuth_ResolveIdentityRejectsDuplicateClaims(t *testing.T) {
	for name, body := range map[string]string{
		"active": `{"active":false,"active":true,"sub":"duplicate"}`,
		"sub":    `{"active":true,"sub":"first","sub":"second"}`,
		"exp":    `{"active":true,"sub":"duplicate","exp":4102444800,"exp":4102444801}`,
		"nested": `{"active":true,"sub":"duplicate","extra":{"claim":1,"claim":2}}`,
	} {
		t.Run(name, func(t *testing.T) {
			allowLoopbackSSRF(t)
			srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
			...
			if id, ok := provider.ResolveIdentity("caller-controlled", "duplicate-token"); ok || id != nil {
				t.Fatalf("ResolveIdentity = (%+v, %v), want duplicate-claim rejection", id, ok)
			}
		})
	}
}
