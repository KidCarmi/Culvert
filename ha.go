...
type standbyLoopState struct {
	...
	localFaultWarned          bool // warn-once latch for a local client/preflight/apply fault (A3)
	ambiguousWarned           bool // warn-once latch for ambiguous leader RPC errors (A8)
}
...
func (s *standbyLoopState) syncRecovered() {
	...
	s.localFaultWarned = false
	s.ambiguousWarned = false
}

// warnLocalClientFault reports a LOCAL fault (failed TLS preflight, bad/
// undecryptable material, client-construction/dial failure, or leader-state apply
// failure) that blocks sync. It is operator-visible but MUST NOT advance the
// promotion streak — a local fault is never evidence the leader is unreachable
// (A3, owner #1/#4). Warn-once per episode to avoid 5s-tick spam.
func (s *standbyLoopState) warnLocalClientFault(err error) {
	...
	go alerts.Fire("ha_local_client_fault", alerts.Payload{
		...
		Host:   s.leaderAddr,
		Detail: "local TLS material / preflight / client construction / leader-state apply failed; standby staying read-only; automatic promotion withheld",
		Source: "ha",
	})
}
...
func (s *standbyLoopState) tick() bool {
	if s.ctx.Err() != nil {
		// The loop is being stopped (stopCh watcher cancelled the context) — an
		// aborted RPC is shutdown, NOT evidence of a dead leader. Counting it could
		// manufacture the threshold failure and trip onMaxFail into a promotion
		// (role flip + lease grab) on a node that is going away.
		return true
	}
	if !s.ensureLocalTLSOK() {
	...
	if s.client == nil {
		...
		if err != nil {
			// NewDataPlaneClient is purely local: grpc.NewClient is lazy (no dial)
			// and buildClientTLS only reads/decrypts local material. Any failure is
			// a LOCAL fault, never proof the leader is unreachable — do NOT advance
			// the promotion streak (A3). Keep retrying, read-only.
			s.warnLocalClientFault(err)
			return false
		}
		s.client = c
...
// syncFromLeader calls HASync on the leader and applies the state bundle. The
// returned reachability outcome distinguishes a live-but-rejecting leader and an
// ambiguous/local fault from a proven transport failure, so local disk/config
// faults and ambiguous errors cannot trigger automatic promotion (ADR-0012 §6.2).
func (h *HAState) syncFromLeader(ctx context.Context, client *DataPlaneClient, token string) (ok bool, outcome reachabilityOutcome) {
	...
	if err != nil {
		...
		return false, classifyLeaderReachability(err)
	}
	...
	if err := json.Unmarshal(raw, &bundle); err != nil {
		...
		// this is a protocol rejection, never a transport failure.
...
const (
	...
	// outcomeRemoteRejected: the leader answered but rejected/failed the call (a
	// malformed or epoch-stale bundle). The leader is ALIVE, so this resets the
	// streak — an integrity/rejection signal, never availability.
