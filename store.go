...
func (c *Config) LoadUIUsersFile() error {
	...
	for _, rec := range records {
		...
		// SEC-RBAC-ROLE-1: a role this build does not enroll never enters the roster
		// verbatim — clamp to least privilege and remember the raw value for the writer.
		rawRole := rec.Role
...
func rosterEnvelope(users map[string]*uiAdminUser, outcome string, revision, securityCounter int64) uiUsersFileEnvelope {
	...
	for name, u := range users {
		...
		role := u.role
		if u.persistedRole != "" {
			// SEC-RBAC-ROLE-1: never serialize the compatibility clamp — the
			// raw role a newer build assigned is written back verbatim so a
			// downgrade never destroys it; an explicit role assignment
			// (applyRosterSet) clears persistedRole and so writes the new role.
			role = u.persistedRole
		}
