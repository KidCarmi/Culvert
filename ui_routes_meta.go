...
var uiRoutes = []uiRouteMetadata{
	...
	{Path: "/api/auth/users", Handler: "apiAuthUsers", Domain: "auth", Public: false,
		Methods: []uiRouteMethod{
			{Method: "GET", MinRole: RoleAdmin, Note: "FE-6A.0: dispatches to per-method helpers (requireRoleJSON inside)"},
			{Method: "POST", MinRole: RoleAdmin, Mutating: true, AuditExpected: true, Note: "create only (409 user_exists)"},
			{Method: "PUT", MinRole: RoleAdmin, Mutating: true, AuditExpected: true, Note: "fenced update (roster revision)"},
			{Method: "DELETE", MinRole: RoleAdmin, Mutating: true, AuditExpected: true, Note: "fenced delete (roster revision)"},
		}},
	...
	{Path: "/api/idp/discover", Handler: "apiIdPDiscover", Domain: "auth", Public: false,
		Methods: []uiRouteMethod{{Method: "POST", MinRole: RoleAdmin, Mutating: true, AuditExpected: true,
			Note: "FE-6A.0: audited (idp.discover, issuer host + outcome) — an admin-actuated outbound fetch"}}},
	...
	{Path: "/api/idp/", Handler: "apiIdPRouter", Domain: "auth", Public: false,
		Methods: []uiRouteMethod{
			...
			{Method: "PUT", MinRole: RoleAdmin, Mutating: true, AuditExpected: true, Note: "delegates to apiIdPUpdate (requireRoleJSON admin inside); fenced on the entry revision"},
			{Method: "DELETE", MinRole: RoleAdmin, Mutating: true, AuditExpected: true, Note: "delegates to apiIdPDelete (requireRoleJSON admin inside); fenced + 409 referenced"},
		}},
	...
	{Path: "/api/certs/upload", Handler: "apiCertsUpload", Domain: "security", Public: false,
		Methods: []uiRouteMethod{{Method: "POST", MinRole: RoleAdmin, Mutating: true, AuditExpected: true,
			Note: "FE-6B.0: fenced (?caRevision= / ?uiCertRevision=), operation-identified import/replace; ?dryRun=1 validates and reports the candidate facts without a fence, an intent or a write"}}},
	...
	{Path: "/api/ca/rotate", Handler: "apiCARotate", Domain: "security", Public: false,
		Methods: []uiRouteMethod{{Method: "POST", MinRole: RoleAdmin, Mutating: true, AuditExpected: true,
			Note: "FE-6B.0: fenced (?caRevision=), operation-identified (?operationId=), challenge-bound rotation; the success audit is operation-keyed and emitted after the durable commit"}}},
	{Path: "/api/ca/rotate/challenge", Handler: "apiCARotateChallenge", Domain: "security", Public: false,
		Methods: []uiRouteMethod{{Method: "POST", MinRole: RoleAdmin, Mutating: true, AuditExpected: true,
			Note: "FE-6B.0: issues the server-owned rotation challenge bound to actor + operationId + caRevision + expiry; process-local ceremony state, mutates nothing durable; audited ca.rotate_requested (the admin action, not a success claim)"}}},
	{Path: "/api/ca/operations/", Handler: "apiCAOperations", Domain: "security", Public: false,
		Methods: []uiRouteMethod{{Method: "GET", MinRole: RoleAdmin,
			Note: "FE-6B.0: authoritative lookup of a durable certificate/CA/OCSP operation intent (pending/committed/aborted/outcome_unknown); settles a pending intent from the object's own evidence; admin-only because the record names the actor"}}},
	{Path: "/api/certs/ui", Handler: "apiCertsUI", Domain: "security", Public: false,
		Methods: []uiRouteMethod{{Method: "DELETE", MinRole: RoleAdmin, Mutating: true, AuditExpected: true,
			Note: "FE-6B.0: fenced (?uiCertRevision=), operation-identified delete of the persisted admin-UI certificate pair"}}},
	{Path: "/api/certificates", Handler: "apiCertificates", Domain: "security", Public: false,
		Methods: []uiRouteMethod{{Method: "GET", MinRole: RoleViewer,
			Note: "FE-6B.0: node-local certificate inventory read model (CA, UI cert, mTLS client cert, OCSP desired/runtime, operation-ledger posture, backup facts); bounded classes only"}}},

	// ── Security: OCSP, GeoIP ─────────────────────────────────────────────
	{Path: "/api/ocsp", Handler: "apiOCSPConfig", Domain: "security", Public: false,
		Methods: []uiRouteMethod{
			{Method: "GET", MinRole: RoleViewer, Note: "FE-6B.0: desired vs runtime posture + revision + bounded mTLS client-cert reason (never a path or a raw error)"},
			{Method: "POST", MinRole: RoleAdmin, Mutating: true, AuditExpected: true, Note: "FE-6B.0: fenced (?ocspRevision=), operation-identified, persist-before-apply durable set"},
		}},
