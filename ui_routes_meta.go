...
var uiRoutes = []uiRouteMetadata{
	...
	{Path: "/api/mcp/servers", Handler: "apiMCPServers", Domain: "mcp", Public: false,
		Methods: []uiRouteMethod{{Method: "GET", MinRole: RoleViewer}}},
	{Path: "/api/mcp/servers/refresh", Handler: "apiMCPServerRefresh", Domain: "mcp", Public: false,
		Methods: []uiRouteMethod{{Method: "POST", MinRole: RoleAdmin, Mutating: true, AuditExpected: true,
			Note: "authenticated peer observation (blocker 11 §7): dials the registered server and re-ingests its tools/list as peer-observed. Admin-only because it can move a tool's catalog drift/eligibility projection. Grants no execution authority and never arms, activates or promotes."}}},
