...
func TestBenchGate_AutoExcludeResolveAllocs(t *testing.T) {
	...
	autoExclude().Observe(foScope, scopeGen(t, "fo"), "fo", "hit.example", autoexclude.ReasonUnsupportedParams, "id:u1")
	if _, ok := autoExclude().Contains(foScope, scopeGen(t, "fo"), "hit.example"); !ok {
