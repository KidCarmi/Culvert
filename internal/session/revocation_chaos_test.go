...
// CHAOS-68 — the session revocation plane.
//
...
func TestChaos68_UserRevocationSurvivesARestart(t *testing.T) {
...
func TestChaos68_UserRevocationCrossesTheGossip(t *testing.T) {
...
func TestChaos68_UserEntriesHaveDistinctTokens(t *testing.T) {
	r := NewRevocationList()
	r.RevokeUser("alice")
	r.RevokeUser("bob")
	r.RevokeUser("carol")
...
func TestChaos68_UserRevocationTokenCannotCollideWithACookiePayload(t *testing.T) {
...
func TestChaos68_UserRevocationSurvivesAHopThroughAnOldNode(t *testing.T) {
	src := NewRevocationList()
	src.RevokeUser("dave")
	...
	if !dst.IsUserRevoked("dave") {
		t.Error("user revocation was erased by a hop through a node predating CHAOS-68")
	}
}
...
func TestChaos68_PersistedDocumentStaysDowngradeParseable(t *testing.T) {
	chaosRevocationsPath(t)
	r := NewRevocationList()
	...
	if err := json.Unmarshal(data, &legacy); err != nil {
		t.Fatalf("a binary predating CHAOS-68 cannot parse the document: %v", err)
	}
	var sawToken bool
	for _, e := range legacy {
		if e.Token == "tok-c" {
...
func TestChaos68_CorruptFileIsReportedAsCorrupt(t *testing.T) {
...
func TestChaos68_UnreadableFileIsNotReportedAsCorrupt(t *testing.T) {
	if os.Geteuid() == 0 {
...
func TestChaos68_PersistFailureIsObserved(t *testing.T) {
...
func TestChaos68_PanickingObserverIsContained(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "revocations.json")
	if err := os.Mkdir(path, 0o750); err != nil {
		t.Fatalf("mkdir: %v", err)
...
func TestChaos68_HealthySaveDoesNotChargeTheObserver(t *testing.T) {
	chaosRevocationsPath(t)
	var seen int
	SetPersistFailureObserver(func(error) { seen++ })
	t.Cleanup(func() { SetPersistFailureObserver(nil) })
...
func TestChaos68_ExpiredUserRevocationsArePruned(t *testing.T) {
	r := NewRevocationList()
	r.mu.Lock()
...
// a gossip round trip.
func TestChaos68_MergeKeepsTheLaterUserExpiry(t *testing.T) {
	r := NewRevocationList()
...
func TestChaos68_UnrevokedSessionsStillDecode(t *testing.T) {
	chaosRevocationsPath(t)
...
func TestChaos68_SuccessfulSaveIsObservedAsRecovery(t *testing.T) {
	dir := t.TempDir()
...
func TestChaos68_UnconfiguredSaveIsNotARecovery(t *testing.T) {
...
func TestChaos68_PanickingSuccessObserverIsContained(t *testing.T) {
...
func TestChaos68_ConcurrentSavesNeverDropARevocation(t *testing.T) {
	const (
		trials = 12
		savers = 16
	)
...
func TestChaos68_SerializedSaveStillRoundTrips(t *testing.T) {
	path := filepath.Join(t.TempDir(), "revocations.json")
	prev := RevocationsPath()
	SetRevocationsPath(path)
