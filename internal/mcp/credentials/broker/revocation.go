...
func (b *Broker) RevokeVersion(ctx context.Context, id profile.ID, version profile.CredentialVersion) (SafeResult, error) {
	...
	if ok {
		if p, okp := b.provider(prof.Provider()); okp && p.Capabilities().CanRevoke {
			if err := p.Revoke(ctx, provider.RevokeRequest{Profile: id, Provider: prof.Provider(), Version: version}); err != nil {
				// Local use is already blocked; record a sanitized provider failure.
				res.Reason = mcperr.ReasonRevocationFailed
				return res, brokerErr(mcperr.ReasonRevocationFailed, "provider revoke failed; local use remains blocked")
			}
		}
	}
