...
// Reservation identifies one successful Reserve so Release can give the charge
// back to the window it was taken in, and only that window. A zero Reservation
// is inert: Release ignores it, so a caller may defer Release unconditionally.
...
// Reserve atomically admits one attempt against ip's budget and CHARGES it in
// the same critical section, returning false when the window is already full.
//
// It exists because the read-then-act pair it replaces was not a bound at all.
// A caller that must refuse an over-budget request BEFORE doing the work cannot
// use Allow (Allow charges every call, so it would bill the requests the caller
// means to admit), and the obvious answer — a read-only "am I over?" probe
// followed by a charge after the work — is a TOCTOU hole: every request in a
// concurrent cohort observes the same not-yet-incremented count and passes, so
// the number admitted per window is the caller's CONCURRENCY, not Burst. That
// is the SEC-BASICAUTH-3 defect; see ui_basicauth_lockout.go's header.
//
// The charge is therefore taken up front and RELEASED by the caller when the
// work turns out not to have been the kind the budget is meant to bound, which
// is what keeps the gate ahead of the work without billing the wrong callers.
// A held reservation counts against the window for as long as it is held, so
// the effective admission rule is "failures + in-flight attempts < Burst".
func (a *APIRateLimiter) Reserve(ip string) (Reservation, bool) {
	...
	if e == nil || now.Sub(e.windowStart) > RateWindow {
		...
		a.entries[ip] = e
		return Reservation{ip: ip, windowStart: e.windowStart, held: true}, true
	}
	if e.count >= Burst {
		// Deliberately does NOT increment: a refused attempt did no work and
		// must not push the window further out for the attempts behind it.
...
// Keep disarms the reservation so a deferred Release becomes a no-op, for the
// path where the work DID produce the thing the budget counts and the charge
// must stand. It exists so a caller can `defer Release` for panic safety and
// still commit the charge on the ordinary path, rather than choosing between
// the two.
...
// Release gives back a charge taken by Reserve, and is a no-op for a zero,
// already-released or Kept Reservation so callers can defer it unconditionally.
//
// The windowStart match is load-bearing: if the window rolled while the
// reservation was held, the charge belongs to a window that no longer exists,
// and decrementing the CURRENT one would credit an attempt nobody made — which
// on a busy client silently widens the budget. An expired reservation is simply
// dropped.
func (a *APIRateLimiter) Release(r *Reservation) {
	if r == nil || !r.held {
		return
	}
	r.held = false
	...
	defer a.mu.Unlock()
	e := a.entries[r.ip]
	if e == nil || !e.windowStart.Equal(r.windowStart) || e.count <= 0 {
		return
	}
	e.count--
}

// Charged reports how many attempts are currently charged against ip's window,
// for tests and for the state-size observability the admin plane exports.
func (a *APIRateLimiter) Charged(ip string) int {
	a.mu.Lock()
	defer a.mu.Unlock()
	e := a.entries[ip]
