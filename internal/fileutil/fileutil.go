...
// ── Two exclusive-write primitives, deliberately distinct (FE-6AR merge 12) ──
//
// This package now carries TWO ways to create a file exclusively, and they
// answer different questions:
//
//   - PublishExclusive (publish.go, FE-6A.2 round 3): first-writer-wins
//     PUBLICATION of a value that must exist EXACTLY ONCE for the life of
//     the store — the IdP candidate-commitment key. A concurrent loser is
//     told the winner exists (created=false) and REUSES the winner's bytes;
//     an existing entry is never replaced, never re-moded, never re-minted.
//   - WriteFileExclusive (below, SEC-SECRETWRITE-1): a safe REPLACEMENT of a
//     RENDEZVOUS entry at a predictable path (the CDR "<bundle>.tmp"
//     staging files, the webhook/logstore/upstream key mints) — it removes
//     whatever sits at the path and creates the file with O_EXCL at the
//     requested mode, so a planted symlink or 0666 file can never receive
//     the secret.
//
// Neither is an implementation of the other. Routing the IdP key through
// WriteFileExclusive would let a second minter REPLACE a key that
// commitments already depend on (the fail-closed `operation_ledger_degraded`
// contract exists precisely so that never happens); routing a rendezvous
// write through PublishExclusive would refuse to supersede a stale staging
// file an interrupted predecessor left behind. Pinned by
// exclusive_join_test.go.
