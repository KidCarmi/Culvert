...
func TestBaseURL(t *testing.T) {
	...
	got, ok := BaseURL(req, false)
	if !ok {
		t.Fatal("BaseURL refused a plain host:port authority")
	}
...
func TestBaseURL_XForwarded(t *testing.T) {
	...
	got, ok := BaseURL(req, true)
	if !ok {
		t.Fatal("BaseURL refused a plain forwarded host")
	}
...
func TestBaseURL_XForwardedIgnoredByDefault(t *testing.T) {
	...
	got, ok := BaseURL(req, false)
	if !ok {
		t.Fatal("BaseURL refused a plain host")
	}
...
func TestEnrollmentAddr(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, ok := EnrollmentAddr(req, tt.grpcAddr, tt.trustFwd)
			if !ok {
				t.Fatalf("EnrollmentAddr refused a plain authority (host=%q grpc=%q)", tt.host, tt.grpcAddr)
			}
			if got != tt.want {
...
func TestRenderScript_And_Compose(t *testing.T) {
	...
	sb.Reset()
	enrollURL := "culvert://enroll/cp.example.com:50051/tok123?ca-fp=sha256:" + strings.Repeat("ab", 32)
	if err := RenderCompose(&sb, "img:1", enrollURL); err != nil {
