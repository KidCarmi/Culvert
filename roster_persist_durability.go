...
func refuseRosterChange(w http.ResponseWriter, r *http.Request, action, object string, err error) bool {
	...
	noteRosterPersistRefused(action+" for "+object, err)
...
// noteRosterPersistRefused records an administrative roster mutation that was
// REFUSED because its durable write did not land — the counter and the log
// line refuseRosterChange charges for the legacy-shaped handlers, factored out
// so the FE-6A.0 roster transaction (commitRoster, store_roster.go — the commit
// point behind create/update/delete/change-password, which never mutates the
// live roster before the write lands) charges the SAME counter. Without this,
// culvert_admin_roster_persist_failures_total would read zero on exactly the
// handlers an operator reaches for during an incident (record 6ARR, merge 8).
// The audit trail of the refusal stays with the handler, which alone knows the
// action and the object.
func noteRosterPersistRefused(action string, err error) {
	n := rosterPersistRefused.Add(1)
	logger.Printf("UIUsers: REFUSED %s — roster not persisted (%d since boot): %v", sanitizeLog(action), n, err)
}
