...
func registerSecurityRoutes(mux *http.ServeMux) {
	...
	// ── CA management (FE-6B.0 backend-truth gate: ui_certificates.go) ──
	mux.HandleFunc("/api/ca/status", apiCAStatus)                    // GET — CA info + cache + rotation + dual-CA (bounded classes)
	...
	mux.HandleFunc("/api/ca/rotate", apiCARotate)                    // POST — fenced, operation-identified, challenge-bound rotation
	mux.HandleFunc("/api/ca/rotate/challenge", apiCARotateChallenge) // POST — server-issued, bound rotation challenge
	mux.HandleFunc("/api/ca/operations/", apiCAOperations)           // GET — durable operation lookup (settles pending intents)
	mux.HandleFunc("/api/certs/ui", apiCertsUI)                      // DELETE — fenced, operation-identified UI-cert delete
	mux.HandleFunc("/api/certificates", apiCertificates)             // GET — viewer inventory read model (node-local)

	// ── OCSP management ─────────────────────────────────────────────────
	mux.HandleFunc("/api/ocsp", apiOCSPConfig) // GET status / POST fenced durable set
