...
//
// ADAPTED TO THIS TREE (FE-6A.0 correction, Blocker 2): a LOCAL session's role
// is never the cookie's — the middleware resolves it from the durable roster
// record and refuses any local session whose security generation does not
// match that record — so a cookie-carried role is only decisive for a
// NON-local (federated) session. The cookie-role gates therefore mint a
// federated session; the local, record-decided path is driven by
// driveLocalMiddleware. The assertions are main's originals.
func driveMiddleware(t *testing.T, sessionRole string) (int, UIRole, bool) {
	t.Helper()
	return driveMiddlewareAs(t, "oidc", sessionRole, 0)
}

// driveLocalMiddleware drives a LOCAL session for bob at bob's CURRENT security
// generation, so the middleware reaches its role resolution instead of refusing
// the session at the generation fence.
func driveLocalMiddleware(t *testing.T, sessionRole string) (int, UIRole, bool) {
	t.Helper()
	gen, ok := cfg.UserSecurityGeneration("bob")
	if !ok {
		t.Fatal("precondition: bob has a security generation")
	}
	return driveMiddlewareAs(t, "local", sessionRole, gen)
}

func driveMiddlewareAs(t *testing.T, provider, sessionRole string, gen int64) (int, UIRole, bool) {
	t.Helper()
	value, err := encodeSession(&Session{
		Sub: "bob", Provider: provider, Role: sessionRole, Gen: gen,
...
func TestRosterToSession_UnenrolledDiskRoleNeverReachesAdmin(t *testing.T) {
	...
	// A LOCAL session: on this tree the middleware decides the role from the
	// durable record (clamped at load), so the cookie value is irrelevant here.
	code, injected, reached := driveLocalMiddleware(t, string(role))
	if !reached || code != http.StatusOK {
...
func TestRosterLoad_ClampIsNeverPersisted(t *testing.T) {
	...
	// This tree's self-service write is the FE-6A.2 fenced one (store_roster.go):
	// it takes the caller's security generation, not a fallback role, and it
	// commits through the roster transaction itself. The assertion is the
	// original: a password change is not a role assignment.
	gen, known := c.UserSecurityGeneration("bob")
	if !known {
		t.Fatal("precondition: bob has a security generation")
	}
	if _, _, err := c.ChangeUIUserPassword("bob", "N3wSecret!pass", gen); err != nil {
