...
// diskUsage (used %, free bytes, total bytes for the filesystem holding path)
// drives disk protection over the whole volume, not just the logs. Its
// implementation is platform-split — statfs(2) on Unix (diskusage_unix.go),
