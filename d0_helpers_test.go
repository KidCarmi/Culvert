...
func TestD0_RouteInventory_Locked141(t *testing.T) {
	// 178 — reconcile parallel-merge drift: /api/diagnose/support (#834) +
	// /api/decryption/redaction both landed with metadata (parity green) but the
	// count lock stayed at 177. See ui_routes_meta_test.go count history.
	const want = 178
