...
func matchDestNorm(rule *PolicyRule, host, normHost string) bool {
	...
	// membership half was always O(1); the host→category half was a linear scan
	// over every configured pattern until urlcat gained its reverse index, which
	// is why this comment used to claim a cost the code did not have. Both halves
	// are now indexed — see urlcat.Store.LookupHost.
