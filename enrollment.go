...
// installScriptTemplate is served by the CP for one-line node enrollment.
// The {{TOKEN}}, {{CP_ADDR}}, and {{CA_FP}} placeholders are replaced at serve time.
const installScriptTemplate = `#!/bin/sh
set -e

# Culvert Data Plane Node Enrollment Script
# Generated by Culvert Control Plane

TOKEN="{{TOKEN}}"
CP_ADDR="{{CP_ADDR}}"
CA_FP="{{CA_FP}}"

echo "==> Culvert Data Plane Enrollment"
echo "    Control Plane: ${CP_ADDR}"

# Download binary from Control Plane (if not already present)
if [ ! -f ./culvert ]; then
  echo "==> Downloading culvert binary..."
  curl -fsSL -o culvert "https://${CP_ADDR}/api/cluster/binary" \
    -H "X-Enroll-Token: ${TOKEN}"
  chmod +x culvert
  echo "    Downloaded successfully"
else
  echo "    Using existing ./culvert binary"
fi

# Run enrollment
echo "==> Enrolling node..."
./culvert -enroll "culvert://enroll/${CP_ADDR}/${TOKEN}?ca-fp=sha256:${CA_FP}"

echo "==> Enrollment complete! Node is now connected to the cluster."
echo "    To run as a service, see: https://github.com/KidCarmi/Culvert/blob/main/docs/deployment-guide.md"
`
