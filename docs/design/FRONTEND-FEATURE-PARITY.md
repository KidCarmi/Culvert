...
| FE-V02 | `livefeed` (Traffic) | `/api/requests`, `/api/logs{,/retention,/purge}`, `/api/export` | viewer | purge/retention | purge (confirm) | yes | 3 s tick | E (TrafficRuleLink…), A | M — high-churn table, filters, history paging, rule-ID deep link. **Read/query console migrated in FE-4; 2A (Batch 2, 2026-08-22) adds the rule-ID deep link**: rows carrying a stable `ruleId` link to `/app/policies/access-rules?rule=<ulid>` (ID-authoritative, name alone never links), proven end-to-end against a real proxied match. **Residue pending 2A-M**: retention configuration, purge (destructive), and `/api/export` download (Blob ownership). |
