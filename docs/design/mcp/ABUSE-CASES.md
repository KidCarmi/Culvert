...
- **Affected assets:** A-6.
- **Expected control:** MCP-EVENT-001,002,**007** and MCP-OPS-005 (mandatory local encrypted spool + backpressure + the isolated denial lane), with the **two distinct outcomes** of EVENT-MODEL §4a/§4b / ADR-0024 §D-5: **(a)** for write/destructive/config-publication/credential/state-affecting-Management ⇒ **fail closed AND** `critical-durability-degraded` **scoped to one durability domain** (`node × capability × partition`) + alert + integrity-protected loss counter; **(b)** for the **auth-failure / authz-DENIAL** event this attacker targets ⇒ the request is **already denied**, and the event is **attacker-mintable**, so it travels the **isolated denial lane** — pre-queue admission control → attacker-rate-independent coalescing → the `P-DEN` partition, which **MUST NOT** consume the `P-CRIT` reserve. Aggregate-commit failure enters **`denial-lane-degraded` only**, with its own distinct loss counter.
- **Expected event:** the deny event survives as a **counted aggregate** (count + first/last-seen), and the request **remains denied** regardless. If the aggregate cannot be committed the system enters **`denial-lane-degraded`** and alerts — and **no authenticated operation is blocked anywhere**: authenticated critical work in another durability domain (other capability, other node, other tenant) **continues successfully throughout**. For a critical *write* event instead, the triggering operation **fails closed** before its own irreversible action **and** `critical-durability-degraded` is entered for that domain only.
...
- **Test:** event-durability-under-saturation, **including the `MCP-T-075` headline attacker case**: saturate the denial lane with unauthenticated auth failures, then assert a subsequent *allowed* write/high-risk operation in another domain **still succeeds**. **Owner:** SRE/Sec. **Severity:** Critical. **Closure:** zero critical loss demonstrated **and the lockout ATTACK PROVEN TO FAIL** — an unauthenticated flood must leave authenticated allowed critical work in another domain succeeding for the whole run, and must never produce `critical-durability-degraded`. This abuse case is **not** closed by degraded-mode alerting alone, and it is **not** closed by demonstrating that a lockout occurred: *(the superseded pre-#926 form of this case required exactly that, which made a passing result evidence of the defect — see `MCP-T-075`)*.
