...
> ADR **[`docs/adr/0023-mcp-agent-security-gateway-trust-boundary.md`](../../adr/0023-mcp-agent-security-gateway-trust-boundary.md)**.
> This file is retained only as a non-authoritative pointer so existing links do not break. **Do not edit
> the decision content here** — make all trust-boundary and PR-1-entry-decision changes in ADR-0023.

- **Status:** Superseded by ADR-0023 (2026-07-24).
- **What moved:** the Option-B six-section proposal that previously lived here — including the eight
  ...
  PR-1 entry decisions D-2, D-5, D-8, D-9, D-13 added) into ADR-0023.
- **Current ADR status:** ADR-0023 is `Proposed`, awaiting formal ratification by the Architecture Review
...
`docs/adr/` was the human-controlled PR-1 entry step. That promotion has now occurred (ADR-0023). To avoid
two competing authoritative records (a repository invariant — see ADR-0001 supersession practice), the
decision content is kept in exactly one place: **ADR-0023**.

## Where to look now

- **Trust-boundary decisions + D-2/D-5/D-8/D-9/D-13 closures:** [`ADR-0023`](../../adr/0023-mcp-agent-security-gateway-trust-boundary.md).
- **Decision lifecycle / closure records:** [`OPEN-DECISIONS.md`](OPEN-DECISIONS.md).
- **PR-1 entry gate:** [`GO-NO-GO-CHECKLIST.md`](GO-NO-GO-CHECKLIST.md) and ADR-0023 "PR-1 entry gate".
