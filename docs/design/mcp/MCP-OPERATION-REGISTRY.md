...
| Gateway | upstream-server-facing peer | server → Culvert (reverse notif) | notifications/cancelled | admitted | control | request-id resolved ONLY vs Culvert-originated upstream requests in the SAME (upstream) direction | upstream protocol-correlation (requestor dimension) | not catalogued | none | n/a (notification) | proto-lifecycle event / ordinary | none (no wire response; never releases client-leg state) | kernel-terminal | n/a (lifecycle) | PR-1 |
