...
| **PW.9** Configure software to have a minimized attack surface | `MCP-OPS-002` bounded connections/SSE/payload/queue/concurrency with per-entity rate limits; `MCP-CONNECT-001` no unsolicited inbound port | [Normative project requirement] | Load/soak/slowloris/queue-saturation test evidence | SRE / Eng | PR-5 (bounds), post-V1 connector slice (D-8) |
