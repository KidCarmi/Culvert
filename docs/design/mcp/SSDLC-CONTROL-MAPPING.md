...
| V13 API and Web Service | OpenAPI-governed admin API surface for `(Mgmt)` and the Gateway's own API/GUI surface | [FACT] for the existing mechanism (`api/openapi/openapi.yaml`, ADR-0007, `api-contract.yml` + `pr-api-governance.yml` MERGE-BLOCKING breaking-change/client-gen checks, `route-classification.yaml`) — MCP route additions to this contract are [Normative project requirement] | OpenAPI diff + contract-governance CI pass on the first MCP admin-route PR | Eng / Release | PR-9 |
