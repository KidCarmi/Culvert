...
|---|---|---|---|---|---|
| **PO.1** Define security requirements for software | Stable, versioned requirement registry `MCP-AUTH-001..MCP-OPS-005` in `SECURITY-REQUIREMENTS.md`, each with rationale, threat IDs, owner, verification, evidence, gate | [Normative project requirement] | `SECURITY-REQUIREMENTS.md` itself + traceability rows in `TEST-TRACEABILITY-MATRIX.md` | Sec / Eng | PR-0 (registry shipped); enforcement lands per-row PR-2…PR-11 |
...
| **PW.9** Configure software to have a minimized attack surface | **`MCP-PROTO-006/008` parse-time structural bounds (message/envelope size, JSON depth, field/array counts, string + method-name length, per-session parser memory and work budget) — enforced by the protocol kernel at **PR-1**, before any listener exists**; `MCP-OPS-002` **listener/runtime** bounds (live connections, SSE streams, concurrency, queues, event buffers, per-entity rate limits) at PR-5; `MCP-CONNECT-001` no unsolicited inbound port | [Normative project requirement] | **PR-1: structural-limit + fuzz evidence (oversized/depth/field/string rejected at parse; resource-budget assertions)**; PR-5: load/soak/slowloris/queue-saturation evidence | SRE / Eng | **PR-1 (parse-time bounds)**, PR-5 (runtime bounds), post-V1 connector slice (D-8) |
...
| Requirements | `SECURITY-REQUIREMENTS.md` — `MCP-AUTH-001..MCP-OPS-005`, each with MUST/MUST NOT wording, rationale, and gate | [Normative project requirement] | The requirements registry itself | Sec / Eng | PR-0 |
| Design | `THREAT-MODEL.md` (STRIDE, attack tree, residual risk) + [`docs/adr/0024`](../../adr/0024-mcp-agent-security-gateway-trust-boundary.md) (trust-boundary decision + D-2/D-5/D-8/D-9/D-13 closures, **`Status: Accepted`**) | [Normative project requirement] | Design-review evidence (documents, RPRs, tests, independent verification); the hard PR-1 entry gate (ADR Accepted; D-1 + D-15 CLOSED) is **complete** | Sec / Eng / Architecture | PR-0 (artifact); PR-1 entry gate closed 2026-07-31 |
...
| Verification | `gosec`, `govulncheck`, `gitleaks`, `staticcheck` (repo-wide, all PRs); `codeql.yml` (PR path-scoped) | **[FACT, verified at `origin/main`, `.github/workflows/codeql.yml`]** CodeQL's `pull_request` path filter **already includes `internal/**`**, which matches `internal/mcp/**` — so MCP Go code **is analyzed on PRs with NO path-filter change**. What is missing is **blocking status**: `codeql.yml` is **not** a branch-protection-required check, so it runs without gating a merge — making it block MCP PRs is a **repo-settings/branch-protection choice, not a workflow edit**. **[MISSING / Proposed]**: malicious-MCP-server tests, OAuth-negative matrix, DNS-rebinding lab, SSE-exhaustion tests | First green run of each missing test class; (optionally) a branch-protection change record if CodeQL is made blocking — **not** a CodeQL path-glob diff | Sec / Eng | PR-2/3/7/5 (respective test classes — see §7 Gap Register); CodeQL blocking = optional policy choice, PR-1+ |
...
| Intelligence | Attack Models, Security Features & Design, Standards & Requirements | `THREAT-MODEL.md` canonical threat IDs (`MCP-T-001..078` — the range **includes the 18 PR-1 protocol-kernel threats `MCP-T-057..074`** plus RPR-1's `MCP-T-076` (reverse channel) and `MCP-T-077` (admitted-but-unpoliced dispatch), RPR-4's `MCP-T-078` (transport-rejection-path legacy fallback + retained unauthenticated stream, owned by `MCP-PROTO-017`), and the #926 durability threat `MCP-T-075`; all are part of the PR-1 entry gate and must not be excluded from SSDLC/PR-0 evidence review); nine-action policy model (`MCP-POLICY-MODEL.md` Appendix); reason-code taxonomy (`MCP.AUTH`, `MCP.SERVER`, `MCP.TOOL`, `MCP.POLICY`, `MCP.CREDENTIAL`, `MCP.INSPECTION`, `MCP.RATE`, `MCP.SYSTEM`, `MCP.MANAGEMENT`) | [Normative project requirement] | Threat-model and policy-model review | Sec / Eng | PR-0 (models), PR-6 (policy engine implements the nine actions) |
...
| V2 Authentication | `MCP-AUTH-001` (no query-string tokens), `MCP-AUTH-002` (audience validation), `MCP-AUTH-003` (RFC 8707 `resource` requested at the AS; resulting `aud`/introspection audience validated at Culvert), `MCP-AUTH-004` (short-TTL/expiry), `MCP-AUTH-006` (replay protection), `MCP-AUTH-008` (separate OAuth clients per capability) | [Normative project requirement] — [FACT] gap basis: today's OIDC binds audience to `client_id` only (`auth_oidc_flow.go:523`), has no RFC 8707 resource indicator (grep 0), and the bearer-access-token path (RFC 7662 introspection, `auth_oidc.go:166-265`) has **no replay defense** and no DPoP/`cnf` sender-constraint (grep 0) | Negative auth matrix: missing/invalid/query-string/wrong-audience/wrong-resource rejections (over **both** JWT `aud` and opaque-introspection forms, plus an `aud`-less token denied on **every** operation class); **replay evidence = replayed per-request DPoP proof rejected + sender-constraint enforced on high-risk profiles + stolen-token-abuse anomaly/rate-limit signals — NOT one-time-use rejection of an access-token `jti`** (reuse of a still-valid access token is not itself replay; ADR-0024 §D-2 items 7–9) | Sec / Eng | PR-3 |
...
| V5 Validation, Sanitization and Encoding | `(GW)` **`MCP-PROTO-006/008`** structural parse-time bounds (size/depth/field-count/string length, per-session budgets) at the protocol kernel — **PR-1**; `MCP-INSP-001` **semantic** tool-argument schema/value-constraint validation — **PR-7**; `MCP-INSP-002` output size/type/schema bounds with truncation policy — PR-7 | [Normative project requirement] | **PR-1: structural-limit + fuzz tests**; PR-7: semantic-schema + output-truncation tests | Sec / Eng | **PR-1 (structural)**, PR-7 (semantic + output) |
...
| V9 Communications | `(GW)` `MCP-SERVER-002` server TLS/workload identity verified and pinned to the registry entry; `MCP-CONNECT-001` mTLS connector identity | [Normative project requirement] | TLS-identity mismatch tests; connector mTLS verification tests | Sec / Eng | PR-2 (server registry), post-V1 connector slice (D-8) |
...
| API4: Unrestricted Resource Consumption | **`MCP-PROTO-006/008` parse-time structural + per-session resource bounds (PR-1)**; `MCP-OPS-002` listener/runtime bounded connections/SSE streams/queues/concurrency/event buffers with per-entity rate limits (PR-5); `MCP-INSP-001` **semantic** input validation (PR-7) | [Normative project requirement]; [FACT] precedent only: repo-wide sharded rate limiter and pooled relay buffers exist for the SWG data path, not for MCP | **PR-1: structural-limit + fuzz + resource-budget tests**; PR-5: load/soak/slowloris/queue-saturation + SSE-exhaustion tests | SRE / Eng | **PR-1 (parse-time)**, PR-5 (runtime bounds), PR-7 (semantic input) |
...
| Structural-limit + parse-time resource-budget tests (oversized/depth/field/string, parser memory/work budget) | SSDF PW.9, API Security API4 | `MCP-PROTO-006/008` | **PR-1** |
| SSE-exhaustion / resource-exhaustion tests (live listener under load) | SSDF PW.9, API Security API4 | `MCP-OPS-002` | PR-5 |
...
| CodeQL **blocking status** for MCP PRs (branch-protection policy choice — **NOT** a path-scope extension: `internal/**` already covers `internal/mcp/**`) | SDL Verification, BSIMM SSDL Touchpoints | — (repo settings, not a stable requirement ID) | PR-1+ (optional) |
