...
| Config-surface matrix **parse validity + anti-vacuity** — valid GFM table (header == delimiter == every data-row width), **non-empty** parse, expected row count, no duplicate field IDs, and forward+reverse summary↔live parity; **zero parsed rows is an unconditional failure**. Executable now as `predicates/predicate-26.py` with seeded controls (16-vs-17 delimiter, zero-row parse, `provider-ref → RC-6`, `RC-0` row in the `RC-2` summary, duplicate field, missing summary member) and negative controls. | **MCP-CFG-001** | `predicate-26.py` exists and passes when run by hand; **not wired into CI** | PR-1 |
