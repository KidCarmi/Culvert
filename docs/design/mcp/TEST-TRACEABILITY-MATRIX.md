...
| Config-surface matrix **parse validity + anti-vacuity** — valid GFM table (header == delimiter == every data-row width), **non-empty** parse, expected row count, no duplicate field IDs, forward+reverse parity for **every** registry-class summary, and a **complete** census (all vocabulary tokens incl. zero-valued, plus row and sensitive-kind totals); **zero parsed rows is an unconditional failure**. Executable now as `predicates/predicate-26.py` with seeded controls (12 seeds: 16-vs-17 delimiter, zero-row parse, `provider-ref → RC-6`, `RC-0` row in the `RC-2` summary, duplicate field, missing summary member, RC-1 summary disagreement, deleted census claim, falsified zero-valued claim, falsified row total, unknown value kind, live `RC-X` row) and negative controls. | **MCP-CFG-001** | `predicate-26.py` exists and passes when run by hand; **not wired into CI** | PR-1 |
