...
| MCP-T-031 inbound rebinding (listener E2E) | MCP-INSP-009 | Listener binds configured interfaces + host allowlist at accept + invokes the primitive; **each capability's allowlist is independent** | E2E rebinding against a live listener, **per listener**, plus **cross-capability allowlist isolation** (a host approved on one listener is still rejected by the other), plus **per-request revalidation over a reused connection** (keep-alive and H2 multiplexing) | No default public bind; E2E rebinding rejected | SRE/Eng | PR-5 |
...
| MCP-T-044 queue saturation/loss | MCP-EVENT-001,002 | Backpressure + fail-closed + denial-event durability lockout | Queue-saturation + durability, **run on both the Management and Gateway legs**, incl. a **Management-originated** denial-event loss | Zero critical loss; **a Management-originated denial-event loss blocks a subsequent allowed Gateway write/high-risk operation** (lockout is system-wide) | SRE/Sec | PR-8 |
