...
| **Transport-rejection / no-legacy-SSE-fallback gate (blocking; version-set content gated on D-1)** — asserts the RPR-4 transport posture: **legacy `2024-11-05` endpoint negative** (no route/config emits an `endpoint` event); **GET without a valid negotiated session/context → terminal `405`** with **zero** stream allocation; **every security-motivated `400`/`404`/`405` is terminal** so the client's spec-mandated follow-on GET is `405` and retains **zero** streams; **`200` `initialize` counter-offer preferred** over a `4xx` hard reject (a `4xx`, or any thrown connect error under an SDK catch-any wrapper, triggers the legacy probe); **N rejected clients ⇒ zero retained pre-negotiation streams** (PR-5 load); **protocol-era separation** (2025 initialize/session fixtures cannot run on a 2026-era handler; 2026 stateless/discovery fixtures cannot fall into 2025 legacy SSE probing). Tests **MUST** drive official/representative SDK fallback logic, not a hand-written mock. Fixtures whose expected values depend on the **final selected version set are `D-1 BLOCKED`** and MUST NOT be marked green until D-1 closes. Enumerated with primary/SDK sources in [`TRANSPORT-FALLBACK-EVIDENCE.md`](TRANSPORT-FALLBACK-EVIDENCE.md) §9; mirrored in [`TEST-TRACEABILITY-MATRIX.md`](TEST-TRACEABILITY-MATRIX.md) §1c. | MCP-PROTO-017, MCP-PROTO-010, MCP-INSP-009, MCP-OPS-002 | MCP-T-078,042 | **PR-1** (exclusion + terminal-status primitive) / **PR-5** (listener held-stream + load) |
...
enforcement fixtures at **PR-6**, without weakening the PR-1 registry gate. The **twelve RPR-4
transport-fallback blocking fixtures** (#929) are enumerated in
[`TEST-TRACEABILITY-MATRIX.md`](TEST-TRACEABILITY-MATRIX.md) §1c with primary/SDK sources in
[`TRANSPORT-FALLBACK-EVIDENCE.md`](TRANSPORT-FALLBACK-EVIDENCE.md) §9 — legacy-exclusion / terminal-status /
zero-stream properties blocking at **PR-1** (primitive) or **PR-5** (listener), with the version-set-dependent
fixtures marked **`D-1 BLOCKED`**; the PR-1 gate proves the structural/terminal/zero-stream properties only
and does **not** claim the PR-5 runtime assertions or any `D-1 BLOCKED` fixture is green.
