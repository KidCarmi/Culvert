...
| Event durability under saturation (**both capabilities; Management-originated lockout included**) | proposed (target PR-8 **+ Future Management-Mutation Gate (D-13) for the real `state-affecting Management` path — PR-8 stubs it**) | Proposed | Yes, for PR-8 | MCP-EVENT-002 | No |
