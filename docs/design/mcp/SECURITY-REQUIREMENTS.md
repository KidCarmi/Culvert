...
| MCP-INSP-009 | The inbound MCP/SSE **listener** **MUST** bind only to explicitly configured interfaces (never default to unrestricted public ingress), enforce the host-allowlist at accept time, invoke the `MCP-INSP-008` primitive on every connection, and be proven by **end-to-end DNS-rebinding enforcement** against the running listener. | Listener binding + wiring the primitive is a runtime concern that cannot exist before the PR-5 Observe Runtime; the listener does not ship in PR-1. | MCP-T-031,055,052 | SRE / Eng | Bind-interface + host-allowlist-at-accept + **E2E** inbound-rebinding tests against a live listener | No default public bind; allowlist enforced at accept; E2E rebinding rejected | PR-5 |
