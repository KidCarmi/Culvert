...
| MCP-PROTO-013 | Every exceeded limit / rejected message **MUST** be handled by message class. For a message classified as a **request**, the kernel **MUST** emit a defined, bounded JSON-RPC error that leaks no internal state (no stack traces, paths, or credentials). For a message classified as a **notification**, the kernel **MUST NOT** emit any wire response — notifications are one-way, so replying both breaks conformance and turns a cheap notification flood into **reply amplification**; the rejection is instead recorded (decision event + metric) and the resources released. Only when the message cannot be parsed or classified at all may a single bounded error be emitted with `id: null`, and that path **MUST** itself be rate-bounded so it cannot become an amplification vector. The **cleanup guarantee is unconditional**: every rejected message, of any class, **MUST** deterministically release all resources associated with the offending message/session. | Bounded, non-leaky error + deterministic cleanup. | MCP-T-057,062,063,074 | Sec / Eng | Error-shape + cleanup/leak tests | Bounded error; resources released | PR-1 |
