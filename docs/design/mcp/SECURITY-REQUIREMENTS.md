...
| MCP-PROTO-014 | The kernel **MUST** reject **invalid UTF-8** in any decoded string and **MUST** apply one defined Unicode normalization form (**NFC**) to every method name and protocol identifier **before** any comparison, allowlist check, or dispatch. Two identifiers that normalize to the same form **MUST** be treated as identical (no confusable/normalization-sensitive bypass of method/allowlist checks); normalization **MUST** be applied consistently so downstream stages never see an un-normalized identifier. | Invalid UTF-8 and Unicode-normalization/homoglyph confusion in identifiers can bypass allowlist/dispatch or create a parser differential; make identifier handling explicit and normative rather than incidental. | MCP-T-057,065 | Sec / Eng | Invalid-UTF-8 rejection + NFC-normalization + confusable-identifier fixtures | Invalid UTF-8 rejected; identifiers NFC-normalized before compare/dispatch; confusables collapsed | PR-1 |
...
| MCP-EVENT-002 | Loss of authentication/deny/configuration/high-risk events **MUST NOT** occur silently. For the **critical write / destructive / configuration-publication / credential** action classes, if a decision event cannot be durably persisted the operation **MUST fail closed AND** the system **MUST** enter the defined degraded mode with alerting (and increment an integrity-protected loss/degradation counter). Read-only/low-risk classes may proceed only under an explicit degraded-mode policy with alarm + counter + bounded retry, never silently (see EVENT-MODEL §4a / ADR-0024 §D-5). | Critical-event durability — fail-closed and degraded-mode-with-alert are both required for critical classes, not alternatives. | MCP-T-044 | SRE/Sec / Eng | Saturation tests (critical class → fail closed **and** degraded+alert) | Zero critical loss; fail-closed + degraded + alert on critical classes | PR-8 |
