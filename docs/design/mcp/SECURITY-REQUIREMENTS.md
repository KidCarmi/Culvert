...
| MCP-AUTH-003 | The gateway **MUST** validate the resource indicator (RFC 8707) binding the token to the **canonical Culvert-controlled MCP resource URI** — which **MAY** encode the target server ID (e.g. the Culvert resource for `/mcp/gateway/{server-id}`) — and **MUST NOT** accept the upstream business MCP server as the token's resource/audience (ADR-0024 §D-2 items 2–3). Unbound tokens **MUST** be denied for write/high-risk. | Prevent token reuse across resources; Culvert is the OAuth protected resource, the upstream server is a policy/broker input, not the token recipient. **Not present today** (0 matches). | MCP-T-004 | Sec / Eng | Resource-binding negative tests (Culvert URI accepted; upstream-server resource rejected) | Denials for mismatched resource; upstream-server resource rejected | PR-3 |
