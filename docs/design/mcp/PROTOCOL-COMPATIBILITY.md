...
| Cancellation | `[EXT]` whether/how the MCP spec defines an explicit cancellation notification for an in-flight tool call. Intended Culvert behavior regardless: a cancellation **MUST** free the corresponding entry in the bounded `id`-correlation table (§3) promptly, and **MUST NOT** be usable to bypass an already-issued QUARANTINE/DENY decision (cancel-and-retry is not a policy-evaluation bypass). **Direction-scoped ownership (`MCP-PROTO-015`, #925):** a cancellation references only a request issued in the **same direction**, and only the **owning requestor** may cancel it; a same-session **opposite-direction** `id` match has **no effect** (no cancel, no state release); the `initialize` request **MUST NOT** be cancelled; a cancellation arriving **after** the response completed is a tolerated **late race**, not a duplicate-completion fault (reconciles the spec-tolerated post-response cancellation with `MCP-PROTO-012`); a malformed/rejected cancellation **MUST NOT** release legitimate outstanding state. `tasks/cancel` is not admitted in V1 (tasks capability not advertised). See [MCP-OPERATION-REGISTRY.md](MCP-OPERATION-REGISTRY.md) §3. | Mixed — cancellation existing in the spec is `[EXT]`; how Culvert accounts for it is design intent |
