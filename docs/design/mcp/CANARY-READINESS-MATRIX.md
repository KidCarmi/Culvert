...
| 21 | Exact request is provably CREDENTIAL-FREE | `credential_path_required` | `canaryExactRequestFacts` in `productionCanaryActivationInputs` — resolved from the SAME coherent capture as row 20, so the two facts can never describe different inventories. MET requires ALL THREE authoritative credential statements to be empty: the matched decision's `Obligations.CredentialProfile`, the authoritative `registry.ServerRecord.CredentialProfile`, and the `catalog.Fingerprint.CredentialProfile` of the record whose digest the four-eyes approval bound. **The policy layer alone is NOT sufficient and that is the whole point of this row**: the obligation is the only credential statement any enforcement path reads, so policy saying "no credential" while the server record requires one leaves execution taking the no-broker branch and reaching a credential-REQUIRED upstream with NO Authorization header. Row 20 is SATISFIED in that state (`TestCredFreeE2E_ServerCredentialProfileIsRefused` asserts it as a premise), which is why this is a separate row and not a clause on the permit. The REVIEWED layer is structural rather than a fourth field: `CredentialProfile` is a hashed fingerprint field, so a `none -> profile-X` change is a DIFFERENT tool identity and the existing reviewed-target drift machinery refuses the stale activation — no separate credential-drift authority, no request-local check. LIVE state, re-observed each evaluation and never copied into the activation's immutable reviewed snapshot, so a credential profile added AFTER activation makes the next FULL ACTIVATION PREFLIGHT refuse. NOT "un-ready" on the NODE surface: this is a `factActivation` row, so `EvaluateNode` excludes it and `mcpCanaryStatus` can never report it — the drift is caught at the next transition or restart, and on the execution plane as a metered block, not on a status read. Distinct from row 7 (`credential_path_not_ready`), which is NODE-level and asks whether this node COULD do credentials at all. | **satisfiable; unmet whenever any layer declares a credential** |
| 21a | Exact reviewed target is backed by a RECENT AUTHENTICATED peer observation | `peer_observation_not_fresh` | `canaryExactRequestFacts` in `productionCanaryActivationInputs` — resolved from the SAME coherent capture and the SAME clock sample as rows 20 and 21, so the three facts can never describe different inventories or different instants. MET requires ALL of: the catalog record carries a peer observation at all (an operator-seeded record never does, and a restart returns every record to that state); the observation names an identity VERIFIED ON THE TRANSPORT that gathered it — never a value read out of the MCP payload, which the peer writes; that identity is still BOTH the catalog record's fingerprint identity AND the registry's current pin (they can genuinely disagree across the repin window, and an observation matching only one is not evidence about the server this node would execute against); the observed target equals the reviewed target EXACTLY on tenant, server, tool, fingerprint and fingerprint format, with tenant taken from the REGISTRY (the peer has no authority over who owns it, which is why `catalog.PeerObservation` carries no tenant field); the server is usable NOW; and the observation is no older than `canary.FirstCanaryPeerObservationMaxAge` (30 min), stamped at or before the evaluation instant. **Row 18 cannot stand in for this and that is the whole point**: `tool_fingerprint_stale` asks whether the reviewed digest still matches Culvert's OWN catalog record, which an operator-seeded record satisfies perfectly and FOREVER, because nothing ever re-observed the peer — so before this row the exact-target facts bound the SEED, not the upstream. Freshness belongs to an exact observed TARGET, never to a server: a fresh observation of F2 says nothing about an activation reviewed against F1, and reports `peer_observation_target_moved` rather than `..._stale`, because refreshing cannot fix a target mismatch. Evidence is gathered ONLY by `execution.Discovery.Discover` over the production upstream transport, reached in production from exactly one governed path (`POST /api/mcp/servers/refresh`), and the observation is stamped BEFORE the request goes out, so a call that stalls past the bound and then succeeds lands already stale. Freshness is TRUTH, not authority: it promotes nothing, approves nothing and arms nothing. LIVE state, re-observed each evaluation and never copied into the activation's immutable reviewed snapshot — it is the one fact that becomes false with NO state change at all, purely by the clock advancing. NOT "un-ready" on the NODE surface: a `factActivation` row, so `EvaluateNode` excludes it and the drift is caught at the next transition or restart, not on a status read. **This row alone did NOT close blocker #11** — an observation that expires AFTER preflight could still back a later request. The runtime half now closes it: `boundaryPeerFreshness` re-asks the SAME verdict (`canary.EvaluateObservedIdentityFresh`, which this row's evaluator delegates to) inside `revalidateTargetTrust`, on one capture and one clock sample, at every pre-send authority re-ask — so the exact target must still carry a fresh authenticated observation at the instant execution authority is spent. See §25f. | **satisfiable; unmet until an authenticated refresh observes the exact reviewed target** |
...
**Node vs activation readiness (two evaluators).** Rows 3, 4, 4a, 16, 17, 18, 19, 20, 21, 21a, 24 (scope
...
exact target peer-observed fresh, budget) are **activation-level**: they are
...
being permanently not-ready because the eleven activation facts default false. `canary.Evaluate`
