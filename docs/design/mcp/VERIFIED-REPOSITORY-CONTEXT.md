...
|---|---|---|---|
| SSRF peer-IP recheck (TOCTOU) | `internal/ssrf/ssrf.go · Control · 126-139`; `PrivateIP · 72-79`; `PrivateHost · 86-113` ⟳ | **Two distinct properties, in two distinct symbols:** dialer `Control` runs AFTER DNS resolution and re-checks the actual resolved peer IP immediately before `connect(2)` (fail-closed on a parse anomaly — it never sees a hostname, so it cannot observe a DNS error); the **pre-flight** `PrivateHost` is what **fails closed on DNS-resolution failure**, rejecting a `LookupHost` error and deliberately not caching it | **Reusable as-is — low-level primitive only** |
