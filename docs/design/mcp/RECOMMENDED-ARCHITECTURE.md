...
| **MCP-EVENT-002** | Loss of authentication/deny/configuration/high-risk events **MUST NOT** occur silently; for the critical **write/destructive/config-publication/credential** classes the operation **MUST fail closed AND** the system **MUST** enter the defined degraded mode with alert + loss counter (both, not either). **The degraded state MUST be scoped to one durability domain** (`node × capability × partition`), be **restart-persistent**, and exit within a **bounded** probe interval (`MCP-OPS-005`). **An authentication-failure or authorization-denial event MUST NOT enter that state or block any authenticated operation** — its request is already denied and the event is **attacker-mintable**, so it is handled in the isolated denial lane (`MCP-EVENT-007`). | `internal/mcp/events` | Fail-closed **and** degraded-mode-with-alert for the critical classes named. **Denial-event branch — a distinct posture, and deliberately a NON-BLOCKING one:** when an authentication-failure or authorization-denial aggregate cannot be persisted, `events` **MUST** raise **`denial-lane-degraded`** with a **distinct** loss counter and keep rejecting the request, and `internal/mcp/runtime` **MUST NOT** block any authenticated operation as a result. Routing this class into the critical lane would let an unauthenticated flood halt allowed writes fleet-wide (`MCP-T-075`) — the evidence control would be reachable by the actor it records. `events` **MUST** also enforce the partition boundary: `P-DEN` **MUST NOT** consume the `P-CRIT` reserve. ordinary/low-risk event loss under backpressure may be handled (under an explicit degraded-mode policy) by sampling/spooling per [`EVENT-MODEL.md`](EVENT-MODEL.md), but the critical classes are never silently dropped. |
