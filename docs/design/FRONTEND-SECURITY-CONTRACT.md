...
| # | Invariant |
|---|---|
| D1 | The three-tier confirmation system is a design-system component (`ConfirmationDialog`): Tier 1/2 styled confirm with impact copy; Tier 3 typed-word gate. The 9 existing Tier-3 ceremonies migrate word-for-word (the count is 10 since 2F-C: the upstream credential clear, `confirm` = the exact entry id; and the PAC DIRECT word became the server-bound candidate challenge in 2F-B — see D15). |
...
| D4 | **[STRENGTHEN]** Two currently-unconfirmed high-impact actions gain ceremonies: `POST /api/releases/dispatch` (Tier 3) and custom-cert upload (Tier 2 with impact copy). *Status: the custom-cert upload half SHIPPED in FE-6B.2 (the `?dryRun=1` review → Tier 2 impact copy naming the persisted target and the restart-only activation of the UI pair; the UI-pair delete is Tier 3 typed; the Root-CA rotation confirms the server-issued challenge with the typed `ROTATE`); the release-dispatch half remains FE-6G.2.* |
...
| D13 | **[ENFORCED — 2D-C]** File Profile and Header Rewrite writes are FENCED, DURABLE, and identity-honest: every mutation asserts the server-owned content-derived revision via `?ifRevision=` (file profiles: `fpv1` from `GET /api/fileblock/profiles/state`; rewrite: `rwv1` from `GET /api/rewrite/state` — the browser NEVER reproduces either), compared inside the owning serialization domain (fileblock store critical section; the AdminSettings `rewriteMutate` domain), with the SAME structured 409 (`error`/`currentRevision`/`yourRevision`) — one dialect across every fenced surface. Identity trust boundary: the browser never fabricates or submits an identity — a rewrite create carries NO `stableId` (the server mints it) and policy rules carry the file-profile NAME as intent (the server stamps `fileProfileId`); the rewrite legacy integer `id` is rendered as process-local metadata and is never used for addressing, deep links, or fencing. Rewrite rules render in EVALUATION order verbatim (order is semantics — the client never sorts the authoritative list, and offers no reorder because none exists in the product); only Create and Delete are offered (no update primitive). File-profile deletes keep the authoritative reference walk (running + active draft candidate) as the only permission decision — the Where Used preflight is information. |
| D14 | **[ENFORCED — 2D-C recovery]** Rewrite management identity is DURABLE-OR-DEGRADED: the settings restore validates the StableID contract like every trust door (empty = legacy backfill; malformed/duplicate refuses the whole slice), and any boot whose identity migration/ledger cannot persist latches a management-identity degradation — GET /api/rewrite/state answers the structured 503 (`{error, degraded:"rewrite-identity", reason}`) with NO ephemeral StableIDs exposed, v2 rewrite mutations refuse, and the v2 page renders the degraded truth (no controls, appliance reason verbatim) while traffic rewrite enforcement continues. Interactive Policy writes require LIVE FileProfile resolution (structured dangling-reference 400) so the modern write door can never manufacture an ID-less legacy reference; the compiled legacy map stays an evaluator fallback for historical rules and a documented bulk-compatibility arm only. |
| D15 | **[ENFORCED — 2F]** Network writes are fenced, ADMIN-ONLY and never claim an unobserved outcome. PAC lifecycle: operation id + `expectedActiveRevision` + durable history incarnation + `expectedActiveSpecDigest` fence, the DIRECT challenge bound to the reviewed candidate; Upstream v2: document-revision fenced create, entry-revision fenced update/delete/credential, write-only credentials sealed under the node-local key, `yaml` entries read-only (409 `yaml_owned`). Every PAC and Upstream mutation (including the manual probe) is `RoleAdmin` in `uiRoutes` — viewers and operators read. An unverifiable 2xx (media type, JSON or action-specific evidence) is UNPROVEN: the ceremony closes, the page latches, the read model is re-read once, nothing is retried; a refusal is a verdict only with its contracted status and typed allowlisted facts; no server string, credential, ciphertext or token reaches the DOM, URL or storage. Recovery markers (PAC lifecycle, CDR enrollment) are non-secret, ownership-bound, cleared at the auth boundary, and NEVER classified — surfaced, dispatched, resolved or cleared — against an unresolved (empty) authenticated subject (2F-G). **[EXTENDED — FE-6A.2]** Identity-provider and administrator writes carry the same contract: document-revision fenced IdP create, entry-revision fenced update/delete, roster-revision fenced account writes, lock-set-generation fenced lockout clears, security-generation fenced self-service password changes; every 2xx is action-bound (identity, revision movement, commit fact, echoed operationId) or UNPROVEN; refusals are per-endpoint typed allowlists (a code an endpoint cannot emit is not a verdict). Write-only secrets (OIDC client secret, LDAP bind password, inline SAML metadata, test credentials, passwords) live only in the open dialog and are released on close/complete/unproven/navigation/sign-out — never returned by a read, never in a URL, marker, storage, log, audit entry or summary, never re-populated from a server response. The legacy-LDAP authority cutover is a typed ceremony on the SERVER-published `cutoverConfirmValue` (`428 cutover_confirm_required` / `409 confirm_mismatch`), bound to provider identity, fence and operationId — binding is never simulated in browser state. The IdP operation recovery marker (`sessionStorage`, subject-bound, non-secret, written before dispatch, read back) is surfaced only for a resolved owning subject; absence is never classified without the ledger's contract evidence (`404 not_found` ⇒ typed re-send bound to the same operationId + candidate digest); `selfAffected:true` completes the auth teardown. **[CORRECTED — FE-6A.2 round 2]** The legacy-LDAP import is a fenced (document revision), operation-identified (`idp.import`), ledger-recorded write with an action-bound answer (`imported:true`, disabled ldap identity/revision, resulting `documentRevision`, echoed operationId, credential-free `source`, fleet facts) and rides the recovery marker (action `import`) like a create; the server's operation binding commits the EXACT secret candidate (keyed HMAC under the node-local `.idp_candidate_key`, never archived) so the same operationId with a different OIDC secret / bind password / inline SAML metadata is `409 operation_mismatch` with zero mutation; every enabled-LDAP create/update crosses the commit-time directory preflight at the write boundary (non-bypassable) and a failure is the bounded `422 preflight_failed` refusal (typed `step` + `reason`, decided before the intent — the marker is released); a re-send dispatches under the RECORDED marker (immutable evidence). **[CORRECTED — FE-6A.2 round 3]** The legacy import is bound to the source the administrator REVIEWED: the read model's server-owned `importSourceRevision` (a keyed, non-disclosing commitment over every security-effective imported field, credential value included) is carried in the import fence, required by the appliance (`428 import_source_required`), refused when stale (`409 import_source_stale`, decided before the intent — a restart on an edited config or a credential-only change), bound into the operation record and echoed; the decoder proves a 2xx (imported or replayed) only for the exact reviewed token and the marker's candidate digest binds it (non-secret). The candidate key is published durably and exclusively and is never re-minted beside a commitment-bearing ledger (degraded, fail closed). The boot retirement is a state machine over every settings-load outcome with a persist-first, operation-keyed, idempotent audit. **[CORRECTED — FE-6A.2 round 4]** Storage recovery never trusts the boot's guess: while the boot observation is pending, every save re-reads the authoritative settings file under the save boundary — a restored durable cutover is adopted verbatim (same identity, no new audit), a still-unreadable or unparseable file REFUSES the save with zero mutation, and a record is minted only when nothing durable exists. The operation lookup is action-discriminated (an `idp.import` record exposes its non-secret `importSourceRevision`, no other action does) and the lost-response recovery clears the import marker and reports commitment ONLY when operationId, action and the exact reviewed token all match the marker (which carries the token) — anything else is UNPROVEN: marker retained, no re-send, no success claim. The candidate key is trusted only inside its confidentiality boundary (regular, non-symlink, owner-only 0600, inspected with non-following metadata); a symlink, non-regular object or group/world-readable mode is `operation_ledger_degraded` and the key is never re-moded or replaced. **[CORRECTED — FE-6A.2 round 5]** Storage recovery never partially adopts: while the boot observation is pending, a settings file that EXISTS at the authoritative path REFUSES every save (readable ⇒ restart required — the runtime booted on defaults and the restart adopts the COMPLETE file, every unrelated durable and unknown-compatible field included, same cutover identity, no new audit; unreadable/unparseable ⇒ refused, zero mutation); only a missing file mints. A `legacy_ldap_retired` sentinel without its record is degraded evidence — retired stays in force, nothing is invented or audited, saves carry it verbatim and never promote it, and the read model reports the bounded `record_missing` (rendered "Record missing", never "Durable"). The candidate key's check and use are bound to ONE opened descriptor (`O_NOFOLLOW` open, `fstat` boundary, exact-length read from that descriptor) so a path swap between validation and read can never make a symlink target's bytes a key. The admin-API cutover's retirement audit is operation-keyed and recoverable: the record is minted audit-pending, the entry is appended with the structural `operationId` only after sentinel + record are durable, exactly once against the durable JSONL record (a failed append stays pending and completes at the next save/boot; a crash after the append never duplicates), and a refused cutover emits nothing. |
