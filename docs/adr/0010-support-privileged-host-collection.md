# ADR-0010: Privileged host/container collection via the maintenance agent (read-only `/v1/collect`)
...
- **Relates to:** ADR-0008 (bundle framework), the maintenance-agent design (`roadmap/D1.6-maintenance-agent-design.md`), the P1.4 pin-binding sudoers model. Full design in `docs/support/SUPPORTABILITY-ARCHITECTURE.md §6`.
...
4. **Ship a support shell/`kubectl exec`-style escape hatch for TAC.** Rejected outright — violates the no-shell rule; remote support (if ever built) is per-command allowlisted, not a shell (ADR-0011 / SECURE-UPLOAD §remote).
