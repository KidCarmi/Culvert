...
| 3 | Full supported-API coverage (document all supported endpoints; retire exemptions) | **IN PROGRESS** — 41/284 documented (22 read endpoints added: policy, authpolicy, default-action, blocklist-mode, ssl-bypass, decryption-health, security, ocsp, session-timeout, connlimit, logger, pac-config), 243 exempt remaining |
| 4 | Runtime contract tests (request/response/failure/authz/tenant) | **CORE DONE** — 27 live response-conformance tests through real handlers; growing with Slice 3 |
