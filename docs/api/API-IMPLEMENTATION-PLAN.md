...
| 3 | Full supported-API coverage (document all supported endpoints; retire exemptions) | **IN PROGRESS** — 51/284 documented (32 read endpoints added: policy, authpolicy, default-action, blocklist-mode, ssl-bypass, decryption-health, security, ocsp, session-timeout, connlimit, logger, pac-config), 233 exempt remaining |
| 4 | Runtime contract tests (request/response/failure/authz/tenant) | **CORE DONE** — 37 live response-conformance tests through real handlers; growing with Slice 3 |
