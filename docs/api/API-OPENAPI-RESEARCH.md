...
| Parse/validate + request/response validation (in `go test`) | **getkin/kin-openapi** | `v0.145.0` (go.mod) |
...
coverage **inside `go test`** with **kin-openapi v0.145.0** cross-checked against
