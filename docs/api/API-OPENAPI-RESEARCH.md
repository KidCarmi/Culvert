...
| Parse/validate + request/response validation (in `go test`) | **getkin/kin-openapi** | `v0.146.0` (go.mod) |
...
coverage **inside `go test`** with **kin-openapi v0.146.0** cross-checked against
