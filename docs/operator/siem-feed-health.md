...
Alert on the **age of the last accepted write**, not on the drop counter. Drops
are a rate that depends on proxied traffic, so a quiet node with a dead
collector barely moves the counter, while the age grows regardless of load:

```
culvert_syslog_up == 0
culvert_syslog_last_success_age_seconds > 300
```

`culvert_syslog_queue_full_total` increasing while `culvert_syslog_up` is `1` is
a different problem: the collector is reachable but slower than this node
