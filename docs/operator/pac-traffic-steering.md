...
storage migration. Steering profiles/proxy pools and the simulator/publish lifecycle
...
# PAC Traffic Steering — Steering Profiles & Proxy Pools (Part 2)
...
lifecycle" and "DIRECT exception governance"). Pools and the ACTIVE steering profile
...
> is still an open naming decision, so it is not a docs-only change.

Each steering profile carries:
...
- a **private-networks behavior** replacing the legacy hardcode: `direct`
  ...
  ranges follow rules/pool). The default steering profile keeps `direct`.

DNS in steering profile PACs is resolved at most once per evaluation (a nested ES3
...
Steering profile PACs are served unauthenticated on both listeners (PAC clients
cannot send credentials) — including plain HTTP on the proxy port, like
`/proxy.pac`. Treat steering profile PAC contents (pool hostnames, routing rules) as
**public information**: anyone who can reach a listener and guess an enabled
steering profile ID can read them.

## Distribution

Steering profiles and pools participate in every config surface: export/import
...
(wire-wipe capable — clearing the last steering profile clears DP state; this PR also
...
fires the `pac_profile_degraded` alert once per steering profile (the latch resets
when the steering profile is next mutated). A secure-mode steering profile whose pool becomes
...
Every control-plane PAC mutation — `/api/pac-config`, steering profile/pool CRUD,
...
plane"): the steering profile store on a DP is control-plane-owned, so a local edit
...
default steering profile keeps working. Clients pointed at `/pac/<id>.pac` receive
...
`POST /api/pac/simulate` (viewer) answers "what would steering profile P return for
...
version, and the steering profile revision.
...
`undetermined_dns` outcome rather than a guess. The `default` steering profile is
simulatable too (its legacy exclusions are expressed as DIRECT rules).

## Draft → Publish → Rollback lifecycle

Each custom steering profile carries a mutable **draft** and an append-only stack of
...
until the admin retypes the server-selected `confirmValue` (the steering profile ID
...
The active steering profile store is the only commit point; the node-local history
records the outcome afterwards, and `GET …/lifecycle` reports
`historyState`: `recorded`, `pending_reconciliation` (the active steering profile IS
...
active steering profile is untouched and still served, but publish/rollback are
refused with `409 history_reset` until an admin acknowledges the loss for
that steering profile with `action: acknowledge_history_reset`, echoing the current
...
steering profile through `POST /api/pac/profiles` / `PUT /api/pac/profiles/{id}` (the
...
the publish lifecycle — both paths share one guardrail. (A brand-new steering profile
has no prior revision, so *any* DIRECT capability counts as new and prompts
once at creation; proxy-only steering profiles save without a prompt.)
...
> **Pool-mutability caveat.** A revision captures the steering profile *spec* (which
...
mints a new revision, updates the active served steering profile, audits
...
Each steering profile (including the synthesized legacy default) is walked for the DIRECT
sources: the always-present plain-host guard (`plain_host` — dotless intranet
hostnames go DIRECT in **every** steering profile; the compiler emits it unconditionally,
so even a secure/proxy-only steering profile bypasses for single-label names — the
steering profile compiler additionally excludes IPv6 literals, the legacy default
...
dependent terminal the static steering profile model cannot otherwise express, injected
...
agent or imported firewall/DNS evidence). `serving` marks enabled steering profiles,
whose bypass is reachable by clients now; a disabled steering profile is inventoried but
serves nothing (404).

## DIRECT exception governance

The inventory answers *"which steering profiles can bypass?"*. Governance answers
...
- `GET  /api/pac/posture/exceptions` (viewer) — one row per DIRECT-capable
  steering profile, joining the inventory (`directCapable`, `serving`, `name`) with its
...
for genuinely DIRECT-capable steering profiles (a steering profile that cannot emit DIRECT carries
no status):
...
- `deltas[]` — each `profile_gained_direct` / `profile_lost_direct` /
  `path_added` / `path_removed`, with the steering profile, the DIRECT `kind`/`detail`,
...
change the *set* of DIRECT paths a steering profile can emit (that is capability, not
...
only appears in the diff when the steering profiles themselves change it.
...
bypasses. Read-only — nothing is published (use the steering profiles editor's
