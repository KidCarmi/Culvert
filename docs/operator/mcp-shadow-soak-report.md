...
† **Scope of the response↔durable parity check.** The response and the durable event are derived from the SAME single `shadowEvidence` per request (evidence‑before‑report; `internal/mcp/execution/responses.go`), so per‑request response↔durable parity is guaranteed **by construction** — a request cannot be handed another request's evidence. It is additionally **asserted per‑request** on the sequential samples (`assertOutcome` coverage, mutation M5, and the churn per‑event `(revision, outcome)` audit). The **concurrent bulk** validates each response against the expected‑outcome oracle and audits **every** committed durable event (count == evaluations, digest verifies, validates, readiness `not_evaluated`), but does **not** re‑correlate individual concurrent responses to individual records — the Shadow response exposes no correlation id, so a per‑response↔per‑record match in the concurrent path is not possible from the response alone. "0 mismatches" therefore means: none found in any per‑request check, and none possible per‑request by construction; it is not a per‑response correlation across all 7,516 concurrent evaluations.
...
| 20 | **No hidden auto‑repair** — corrupt evidence not repaired; unknown schema fails closed; tampered event fails digest; stale not retargeted; changed tool not auto‑reapproved | ✅ PASS — schema v3 fails closed; a tampered event fails `VerifyDigest`; **and, at the real durable seam, a byte flipped inside a committed persisted P‑CRIT segment is detected by `Spool.Recover` (`Corrupt=true`, reason `record verify`) and never silently repaired/discarded/accepted** — with a clean‑reopen control (`TestShadowSoakEvidencePersistedCorruptionFailsClosed`) |
...
- **Fix (harness):** the readers now fully paginate both partitions following `next`, and — critically — a request's durable event is identified by a **pre‑request event‑id snapshot + set difference**, not by "latest". `newShadowEvidence(t, preIDs)` returns the single committed shadow event whose id is new since `preIDs` was captured (failing unless exactly one appeared, which makes the sequential‑request precondition explicit and strengthens the assertion). Selecting by a maximum `Event.TimeUnixNano` was deliberately **rejected**: that field is stamped at event construction, before `Spool.Commit` takes the partition lock, so it is not a commit‑order value and can disagree with commit order under concurrency (across the two independent partition locks). The id‑diff has no dependence on timestamp‑vs‑commit‑order skew.
...
6. **Expected denial parity (Shadow `would_block` == the deny set for the same traffic)?** — **NOT VERIFIED in this build.** Every `would_block` correctly maps to a deny‑class policy decision (asserted per request against the policy the soak published), but the soak checks Shadow outcomes against its own expected‑outcome oracle — it does **not** replay the same traffic through Observe and compare Observe's deny set. The same‑traffic Shadow‑vs‑Observe comparison the criterion names is **not exercised**; establishing it requires an Observe‑parity harness this soak does not include.
7. **Stable latency (p99 within budget; no admission saturation)?** — **NOT ASSESSED.** Latency is recorded as **informational only** (p99 ≈ 27 ms) and §16 deliberately asserts **no SLA/budget** (a performance threshold must not be invented to make this phase pass). With no defined budget, neither the within‑budget comparison nor an admission‑saturation bound is proven. A Canary phase must define and enforce the budget.
...
9. **Kill‑switch drills pass (engage → next evaluation blocked)?** — **Honored, but NOT as the literal `WOULD_BLOCK` outcome.** Under load, 608/608 requests are emergency‑blocked at **admission** (`rollout_emergency_active`, one `evaluation_error` each, 0 `would_execute`, upstream 0, deterministic recovery on clear). This is the implemented fail‑closed behavior — a killed node short‑circuits **before** Shadow evaluation, so no `would_block` evaluation event is produced. The §12 wording ("next evaluation is `WOULD_BLOCK`") describes an evaluation‑path outcome the implementation does not take; the drill proves the kill is *honored* (arguably stronger — blocked before evaluation) but does **not** exercise a literal `would_block`. Counted as not‑strictly‑verified against the criterion's wording.
10. **Restart drills pass (durable evidence survives; no execution replay)?** — YES. Full recovery; LiveExecutor absent; upstream 0.
11. **Observability verified (all shadow series emit; health three‑state correct)?** — YES. `assertOperatorObservability` asserts the real operator surfaces: every `culvert_mcp_shadow_*` series row emitted by the `/metrics` serializer equals the live counters (parsed 1:1, uniqueness‑checked), and the status map's full three‑state health is asserted — `evaluator_composed=true`, `shadow_deps_ready=true`, `reason="composed"`, the node‑readiness **`preflight.Ready=true` with no reasons**, and `live_execution_ready=false`. A regression in any reported health state fails the gate.
12. **Operator procedure tested (runbook dry‑run)?** — **NOT VERIFIED.** The node‑readiness function's result *is* now asserted (`preflight.Ready` on the status surface — criterion 11), but that is not the same as driving the **operator‑facing runbook procedure** end‑to‑end (the documented admin dry‑run flow an operator actually invokes before activation). No such operator flow is executed and checked here. Establishing this criterion requires driving the documented operator procedure and checking its output.
13. **`PREREQ‑MCP‑KILL‑1` CLOSED?** — **NO. OPEN.** See §11.

**Exit Review outcome:** The Shadow‑correctness criteria this Shadow‑only build can exercise end‑to‑end and assert literally — **1, 2, 3, 5, 10, 11** — are satisfied. The remaining seven are **not established here, by construction, deliberate scope, or a wording divergence**, and the Exit Review names each as work a Canary phase must still do:

- **4 — stale‑decision:** `would_fail_stale_decision` is structurally unreachable through the live pipeline (boundary drift is race‑only); needs a driven boundary‑drift stale path.
- **6 — denial parity:** the soak checks its own outcome oracle, not a same‑traffic Shadow‑vs‑Observe comparison; needs an Observe‑parity harness.
- **7 — latency budget:** measured informationally only, with **no** budget asserted (§16); needs a defined, enforced budget.
- **8 — credential‑planning:** no planner is composed, so readiness is never derived from a plan; needs a plan‑only credential path.
- **9 — kill‑switch outcome:** the kill is *honored* (admission emergency‑block, proven under load), but the implementation blocks before evaluation, so the literal `WOULD_BLOCK` outcome the criterion names is not produced.
- **12 — operator runbook dry‑run:** the internal preflight runs, but the operator‑facing procedure is not driven/asserted; needs the documented flow exercised.
- **13 — `PREREQ‑MCP‑KILL‑1`:** a **HARD Canary prerequisite that is OPEN** (§11).

The soak itself passes — all seven hard invariants hold (§2), the kill is honored, and durable evidence fails closed on real on‑disk corruption — but the Exit Review **does not authorize Canary architecture to begin**: it precisely enumerates what a Canary phase must build and prove first.
...
The Shadow Exit Review (§10) confirms every Shadow‑correctness criterion this Shadow‑only build can exercise end‑to‑end and assert literally (**1, 2, 3, 5, 10, 11**) is met, and precisely enumerates what a Canary phase must still build and prove: criteria **4** (a driven boundary‑drift stale path), **6** (a same‑traffic Observe‑parity comparison), **7** (a defined, enforced latency budget), **8** (a real credential‑planning path), **9** (a literal `would_block` kill outcome — the kill is honored today via admission block), **12** (the operator runbook flow driven and asserted), and — the **HARD Canary prerequisite** — **13**, `PREREQ‑MCP‑KILL‑1`, which is **OPEN**. Canary architecture may **not** begin until those are implemented and proven. Accordingly, the `SHADOW EXIT REVIEW PASSED — CANARY ARCHITECTURE MAY BEGIN` line is **deliberately withheld.**
