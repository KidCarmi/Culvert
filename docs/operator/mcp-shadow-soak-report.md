...
12. **Operator procedure tested (runbook dry‑run)?** — **NOT VERIFIED.** The harness drives the internal activation preflight during composition, and `evaluateShadowNodeReadiness` is only incidentally invoked while building `mcpShadowStatus`; its result is not asserted and no operator‑facing runbook procedure is executed end‑to‑end. Establishing this criterion requires driving the documented operator flow and checking its output.
