...
| Attack | Finding |
|---|---|
| TOCTOU: final check vs. call | No blocking/business logic runs between the kill re-read and `Upstream.Call` (§2), and both read the same lock-free generation, so any kill that happened-before the read is honored. An **irreducible instruction-level check-then-act window** remains — a kill engaged strictly *after* the last read but before the call is not observed by that request. It is **accepted, not closeable** without holding a lock across `Upstream.Call` that `EngageKillSwitch` also takes, which is rejected on two grounds: it would place a mutex across a network I/O side effect so a slow/hung upstream would make the operator's emergency kill itself block (inverting the stop's purpose), and §2 forbids inserting synchronization between the final check and the call. The invariant is "the last executable instruction before the side effect honors the authoritative kill state at that instant", which holds; a kill arriving after that instant was not yet authoritative when the request committed. This residual is the same accepted class as the in-flight-materialization note (§3, §8): the window shrank from the whole durable-commit + credential-materialization span (blockable, ms–s) to a handful of instructions. |
