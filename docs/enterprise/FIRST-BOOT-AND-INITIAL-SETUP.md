...
`/data` is a set of flat JSON/text files created lazily on first write — **no database, no migration engine**. Key files: `ca.bundle`, `ui_users.json`, `policy.json`, `idp_profiles.json`, `audit.jsonl`, `requests.jsonl`, `config_versions/v{N}.json`, `cluster.json`, `threatfeeds.json`. Encryption at rest is opt-in via `CULVERT_CA_PASSPHRASE` / `CULVERT_LOG_PASSPHRASE`. **Careful — an empty passphrase does not disable inspection:** on a fresh install `SaveCA` writes the root CA key as **plaintext PEM** and inspection stays active with an unencrypted key (`internal/ca/ca.go:194-235`) — a key-custody risk, not a safe default. (Inspection only goes tunnel-only when an *already-encrypted* `ca.bundle` fails to decrypt.) Always set the passphrase for any inspecting deployment and confirm `ssl_inspection: ready` on `/health`.
