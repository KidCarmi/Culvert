...
| **T-PROMPT** | Prompt injection via bundle-derived text | B5 | AI gets **normalized findings + approved excerpts by default**, not raw (ADR-0032); untrusted-data delimiting; fixed system policy; TAC-approval gate; no appliance action path (outbound-only) | `TestAIInputNormalizedOnly` |
