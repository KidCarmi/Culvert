...
| RISK-024 | MEDIUM | ✅ CLOSED | Inbound `X-User-Identity` unscrubbed on ingress: on identity-free paths (default-Exempt / scoped exempt / no-backend) a client-supplied value reached `policyStore.Evaluate` as `SourceIdentity` and the HTTP-path log attribution (CWE-290/807) | unconditional ingress delete + direct `authenticatedIdentity` evaluation + trailer scrub (F1, 2026-08-13), pinned by `authz_identity_ingress_test.go`; residual header transport CLOSED by F6 — typed `ProxyIdentity` plumbing through `handleHTTP` + the inspect chain, nothing stamps or reads `X-User-Identity` internally, ingress/egress scrubs retained as defense-in-depth — see `docs/security-reviews/2026-08-13-x-user-identity-ingress-trust.md` |
