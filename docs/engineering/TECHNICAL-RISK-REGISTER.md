...
| RISK-026 | HIGH | OPEN | MCP Gateway has no per-source admission: one client can monopolize a capability's entire worker pool + queue, PRE-AUTHENTICATION | `runtime.Limits.AdmissionBudget` is documented as a "per-source admission budget", validated, ceiling-checked, exposed as an accessor — and has ZERO enforcement call sites; `Listener.admit` has no notion of a source and `runtime.Request` carries no client address. Admission is step 1, so the flood costs nothing to mount. Options + recommendation (two-tier: coarse pre-auth per-TCP-peer, per-principal after auth) in `security-reviews/2026-08-24-mcp-backend-full-review.md` §4 MCP-05b. **Blocker for exposing an MCP listener beyond a controlled host.** | **UPDATE 2026-08-25:** two adjacent amplifiers removed (OVN-07 per-connection request budget — one HTTP/2 socket could occupy the whole worker pool; OVN-06 single credential validation), neither of which is per-source fairness and neither of which closes this. `AdmissionBudget` is now recorded as `reserved` with no enforcement owner in the Limits anti-drift wall (`internal/mcp/runtime/limits_ownership_test.go`), which fails the build if it is ever silently read — so the knob no longer implies a control it does not provide. Implementation-ready options and a recommendation are in `docs/design/mcp/ADR-PROPOSAL-mcp-admission-fairness.md`. **Still OPEN: needs an architecture decision on deployment topology, not a patch.**
| RISK-027 | MEDIUM | CLOSED | A degraded or dead MCP listener is invisible to fleet monitoring | CLOSED 2026-08-25 (`mcp_health_plane.go`). MCP now reports through the same three surfaces as every other subsystem: a `/healthz mcp` field, a REPORT-ONLY `/readyz mcp_gateway` row, and 16 `culvert_mcp_*` series — all emitted ONLY on a node that requested MCP, since `up 0` on a node that never had MCP is indistinguishable from a dead listener (the socks5_health.go paging rule). The operator-contract decision was taken as recommended: the row is report-only and never gates the default readiness verdict, so an MCP fault cannot pull a healthy SWG out of rotation; `/ready?strict=1` (CHAOS-09) remains the existing opt-in for operators who want dependency-degraded nodes ejected. Details on `/readyz` are FIXED strings from a closed set because that surface is unauthenticated on the proxy port. A fire-once `mcp_gateway_down` alert latches per episode, is `HasSubscriber`-gated, carries a BOUNDED state label as its Detail (Dispatch dedups on event+Detail — the WK-12/RS-5 defect), and clears only on OBSERVED evidence of the capability serving. Evidence: `mcp_health_plane_test.go`; see `security-reviews/2026-08-25-mcp-overnight-hardening-run.md`. |
...
| RISK-029 | MEDIUM | ✅ CLOSED | A password change silently de-enrolled the account's TOTP second factor (and reset the OTP replay counter) | `Config.SetUIUser` / `Config.SetAuth` replaced the whole `uiAdminUser` record on every credential write, dropping `totpSecret`/`backupCodes`/`totpLastCounter` — durably, unaudited, with no proof of possession of the authenticator. Reachable from `POST /api/auth/change-password` (viewer-and-up, self), `POST /api/auth/users` (admin, any account) and `POST /api/settings/auth`. `ClearTOTP`, the intended de-enrolment primitive, had NO production call site, so the only removal in the tree was an unintended consequence of struct replacement. Closed 2026-09-18 by `newUIAdminUserPreservingTOTP` (`store.go`), the single constructor every credential write now goes through; `--reset-password` keeps its break-glass clear, now explicit and announced. Correction round (Codex, PR #1429): the replay counter belongs to the SECRET, so `ClearTOTP` now clears `totpLastCounter` and `SetTOTPSecret` resets it when the secret CHANGES (conditionally — re-issuing backup codes for the same secret must keep it, or the fix reopens the replay window it protects). It had been zeroed only as a side effect of the record replacement this change removes, so without it a de-enrolled account could not re-enrol until wall-clock time passed a counter from the lost device. Gates: `auth_totp_preservation_test.go` (17, each defect gate RED against the pre-fix tree, plus two mutation proofs and a structural wall). Review: `security-reviews/2026-09-18-admin-second-factor-preservation-window.md` |
| RISK-029 | HIGH | ✅ CLOSED | Admin credential brute-force bound applied to ONE of four entry points: `uiAuthMiddleware`'s Basic Auth fallback, the PUBLIC `GET /api/auth/status`, and the SSE revalidation each verified credentials with no lockout, no failure record, no audit entry and no rate limit (`securityMiddleware` throttles only mutating requests). `/api/auth/status` is on `isPublicUIAuthPath`, so an UNAUTHENTICATED caller had an unthrottled password oracle that also cost one bcrypt of appliance CPU per probe (CWE-307/CWE-770/CWE-778, OWASP A07+A09) | CLOSED 2026-09-14 — one chokepoint `verifyUIBasicAuth` (`ui_basicauth_lockout.go`) applying the SAME two-tier `loginLimiter` as `apiAuthLogin`, checked BEFORE verification so a locked attempt costs no bcrypt, sharing state with the login endpoint so endpoints cannot be alternated, auditing one `auth.lockout` per TRIP (never per attempt — the CHAOS-63 amplifier), surfaced as `culvert_admin_basic_auth_lockout_refused_total`. Gates: `ui_basicauth_lockout_test.go` (5 defect gates verified failing against the pre-fix shape + 4 controls) and two enumeration walls in `ui_basicauth_wall_test.go`. Runbook: `docs/operator/admin-api-credential-lockout.md` |
| RISK-030 | MEDIUM | CLOSED | HTTP Basic Auth on the admin plane did not enforce TOTP: `apiAuthLogin` required the second factor via `verifyLoginTOTP`, but the Basic paths never did, so a valid PASSWORD alone reached the full admin API at the user's role — an MFA bypass for every TOTP-enrolled admin (CWE-287, OWASP A07:2021) | CLOSED by #1420 / SEC-BASIC-1 (`ui_basic_auth.go`), which refuses the Basic scheme outright for an account with TOTP enrolled rather than silently downgrading — the Basic scheme carries one credential, so there is no third option. The refusal comes AFTER the password check so it is not an enrolment oracle, and is deliberately not charged to the per-account lockout so a caller replaying Basic cannot lock the real operator out. Breaking for anyone scripting against a TOTP-enrolled account: see `docs/operator/admin-basic-auth.md`. Reported from the SEC-BASICAUTH-1 review (2026-09-14), which deliberately left it as needing an owner decision |
| RISK-031 | LOW | OPEN | Unauthenticated lockout-state growth on the admin plane (**AU-17b**): `GET /api/auth/status` is on `isPublicUIAuthPath` and is not a mutating request, so nothing rate-limits it, and each failed Basic-Auth attempt creates a tier-1 pair plus a tier-2 account entry keyed by a CALLER-CHOSEN username (measured: 800 entries from 400 requests). Growth is in entry COUNT only and never affects whether a valid credential is accepted (CWE-770) | Bounded per KEY by `internal/lockout`'s injective `boundUsername` clamp (`MaxUsernameKeyLen` — a 4 KiB username produces the same key footprint as a short one, ~4000x smaller than CHAOS-63's unbounded key), in TIME by `Cleanup`'s window (wired into the shared janitor), and newly VISIBLE via `culvert_login_limiter_entries` — the admin plane's first signal for lockout-map growth. A per-client refusal was tried for this in review and WITHDRAWN (SEC-BASICAUTH-4): keyed on the client, it let an unauthenticated flood from a shared egress deny a VALID credential for every admin behind that address. Designed fix: `internal/authstate`'s fair-share eviction ported into `internal/lockout` — evict the oldest entry of the client key holding the MOST, so a flooding source evicts ITSELF; it bounds state WITHOUT REFUSING ANY REQUEST and also closes AU-17 (no entry-count cap, because evicting at a cap evicts a REAL lock). **Any bound here must delay or evict, never deny an unverified request.** Pinned as a fact by `TestSECBASIC4_ResidualUnauthenticatedStateGrowth`; see §1c of `docs/engineering/security-reviews/2026-09-14-admin-plane-credential-entry-points.md` |
| RISK-032 | LOW | OPEN | Concurrent credential verification on the admin plane is unbounded (**AU-18**): below the lockout threshold, simultaneous Basic-Auth attempts from one client each run a `bcrypt.DefaultCost` comparison (~80 ms of exclusive CPU) with nothing capping how many run at once (CWE-770) | The lockout still caps attempts per client per window, and a LOCKED attempt still costs no bcrypt (that bound is `loginLimiter.Check`, unaffected). What is missing is a concurrency cap. Designed fix: route this path through `internal/authcost`, the governor CHAOS-57 built for exactly this and which recorded the admin plane as *"deliberately left: bounded by loginLimiter instead"* — it makes an over-cap client WAIT for its own earlier verification instead of refusing it, which is CHAOS-57's own finding that *"one source, one slot" is a correct FAIRNESS rule and an incorrect ADMISSION rule*. A per-window REFUSAL was tried instead and withdrawn (SEC-BASICAUTH-4 — it denied valid credentials on a shared egress), so the replacement must be the waiting shape, not a second refusal. Interim mitigation: restrict the admin port with `-ui-allow-ip`. See §1c of `docs/engineering/security-reviews/2026-09-14-admin-plane-credential-entry-points.md` |
| RISK-033 | LOW | OPEN | Replay of a COMPROMISED first factor against a TOTP-enrolled account is unbounded (**AU-19**): the Basic scheme can never succeed for such an account, but each attempt still costs one `bcrypt.DefaultCost` comparison and one durable `auth.basic.refused` audit line, with nothing capping the rate (CWE-770, CWE-778) | Requires the attacker to already hold a VALID password for an enrolled admin, so it is strictly downstream of a credential compromise — and the audit lines it writes are the evidence an operator wants in exactly that incident. #1420 bounded it with a per-IP failure budget; that budget was REMOVED (SEC-BASICAUTH-4) because, keyed on the client, it denied valid credentials to every admin behind a shared egress, which is the worse of the two. Not replaced with a fourth mechanism in the same change that removed one for being wrong. Designed fix: the bcrypt half is AU-18's (`internal/authcost`, which WAITS rather than refusing); the audit half is a per-(IP, account) dedup on the refusal entry, which denies nobody since no legitimate client for an enrolled account can use Basic at all. Interim mitigation: `-ui-allow-ip` |
