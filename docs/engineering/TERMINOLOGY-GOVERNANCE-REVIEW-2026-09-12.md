...
keys) is also unchanged (`config.go:237,250,261-277`). The reason-code strings sharing the same
unqualified "qualification" prefix (`mcp_observe_startup.go:171-213`, `mcp_policy.go:145-341`:
`qualification_inventory_invalid`, `qualification_policy_uncompilable`, `qualification_policy_traversal`,
etc.) are part of the same gap but are PRE-EXISTING — identical at both ends of `d378dff..2833db3`
(`mcp_policy.go` has no diff in the window, and `mcp_observe_startup.go`'s only change is unrelated
`CanaryOperationClass` wiring). An earlier draft of this report attributed them to this window; that was
...
- **Affected API:** `api/openapi/openapi.yaml`/`openapi.json` — an additive MINOR contract change
  ...
  working. The same change must also neutralise the "An accusation" wording in the
  `unauthorizedResponderTotal` and `notForCertificateTotal` descriptions (`openapi.yaml:488-489`, carried
  into `openapi.json`). Both counters can rise from an ordinary CA-side fault, so the API contract must
  not keep the claim the GUI banner fix removes. These are the requirements, NOT an exhaustive checklist: the implementing change owns the exact
  steps, including every generated artifact downstream of the contract (e.g. the committed frontend types
  `frontend/src/api/types.gen.ts`, regenerated by `frontend/scripts/generate-types.sh`), and the
  repository's API-governance and frontend drift gates enforce them.
...
T-39 (unchanged by this window — see above). Full descriptions and the priority-ordered refactoring plan are unchanged from
...
form is wrong there, so both mentions are corrected in this PR. The same name in the roadmap's CHAOS-65 "What shipped" list
(`roadmap/CHAOS-ENGINEERING-REVIEW.md:6136`) is corrected too.
