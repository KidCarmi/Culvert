...
> - **`internal/mcp` subpackage count — CORRECTED, 2026-09-08.** `find internal/mcp -mindepth 1
>   -maxdepth 1 -type d | wc -l` = **27** (25 as of 2026-08-24, before the `tooltrust` package
>   landed 2026-08-28 for ADR-0034 and the `canary` package landed 2026-08-29 for ADR-0035). §2's
>   ADR-0024 row is updated to 27; `CLAUDE.md` and `roadmap/CHAOS-ENGINEERING-REVIEW.md` §"Still
>   unguarded: the MCP runtime listener" carried the same stale count and are corrected in the same
>   pass. Fact-check only — no maturity score re-judged.
...
| Admin-plane credential entry points review (2026-09-14) | `docs/engineering/security-reviews/2026-09-14-admin-plane-credential-entry-points.md` | ✅ Complete. Found the admin plane had **four** credential entry points and a brute-force bound on **one**: `uiAuthMiddleware`'s Basic Auth fallback, the PUBLIC `GET /api/auth/status` and the SSE revalidation each verified credentials with no lockout, no failure record, no audit entry and no rate limit (`securityMiddleware` throttles only mutating requests) — an unauthenticated password oracle costing one bcrypt of appliance CPU per probe. Closed by one chokepoint sharing the login form's two-tier lockout (RISK-029). Also removed a public-allowlist PREFIX that pre-authorised unwritten TOTP enrolment routes (SEC-PUBLICPATH-1, closing GAP-2). **RISK-030 registered and OPEN**: Basic Auth still does not enforce TOTP, so a valid password alone reaches the full admin API |
