...
- **Current behaviour:** The only code-supported recovery is the one-shot CLI flag `--reset-password username:newpassword`, which runs the binary on the host (e.g. `docker exec`/`compose run`) and rewrites `ui_users.json`. There is no API, GUI, network, email, or console reset. There is no default/emergency credential. External IdPs never govern admin login (admin UI is local-bcrypt only), so there is no IdP-based admin recovery either. Brute-force lockout state is in-memory; it is cleared in-band by the admin-only `POST /api/auth/lockouts`, and by a process restart when nobody can authenticate to reach that endpoint. Note this gap is a lost admin **password**, not a stuck lockout — clearing a lock has an in-band path, recovering a password does not.
- **Evidence:** `main.go:381-404` (`--reset-password`, read directly). `ui_auth.go:65,114,468` (admin login/setup use local bcrypt only). `internal/lockout/lockout.go` (lockout state is in-memory; `ResetUser`, surfaced as `POST /api/auth/lockouts`, clears a lock in-band — it cannot reset a password). Exhaustive negative search for `break-glass|recover|reset-admin|default admin|emergency-admin` found only the CLI flag.
