...
- **Evidence:** No `*.ova/*.ovf/packer/cloud-init/kickstart/preseed/autoinstall` files (exhaustive search, agent-confirmed). `scripts/install.sh:1-233` (Docker Compose installer). `docker-compose.yml:31-233`. `.github/workflows/appliance-catalog-update-e2e.yml` installs via `docker compose up -d`. No systemd unit ships for the proxy process (only `packaging/systemd/culvert-maint.service` for the host agent).
