...
> separate the two reviews, so this pass is narrowly scoped rather than a full re-sweep: (1) confirmed
> by diff inspection that none of the eight carried-over open findings' files were touched in this
> window, so they are re-confirmed unchanged without re-auditing their surrounding clusters from
> scratch; (2) audited the two feature areas that shipped in the window in full — the MCP Agent
...
No regressions: all eight carried-over open findings (T-9, T-11, T-12, T-16, T-17, T-18, T-21, T-25
residual, T-13 residual) are re-confirmed unchanged — `git diff --name-only 8524751..HEAD` touches none
of the files any of those findings cite, so no new code could have widened or narrowed them.
...
vocabulary.** PR-7's inspection hard-fail and PR-8's decision-event commit-failure paths write ad-hoc
literal strings (`"BLOCKED_BY_INSPECTION"`, `"REDACTION_FAILED"`) and snake_case `mcperr.Reason` codes
into fields whose own doc-comment promises only the canonical taxonomy. Zero-consumer today (verified: no
other `.go` file, including tests, reads `PolicyReason`), so this is the same "catch it while it's free"
class of finding as T-32, not a compatibility-risk one.
...
## Wave 1 — Re-confirmed unchanged (no file overlap with this window's commits)

`git diff --name-only 8524751..HEAD` was checked against every file citation in the eight open
carried-over findings — none overlap:

| Finding | Files it depends on | Touched this window? |
|---|---|---|
| T-9 | export/import + parity-test surface (`exportedAt`) | No |
| T-11 | `policy.go`'s default-action vocabulary (pre-F3b-4 sections), `ui_policy.go` core | No (this window's `policy.go`/`ui_policy.go` edits are the F3b-4 `matchCategory`/`lookupHostCategory` source-aware resolution and the new SaaS status routes — a different region of both files) |
...
| T-21 | `cluster_convergence.go`, `configversion.go`'s rollback `cp_version` surface | No (`configversion.go`'s one edit this window is the unrelated F3b-4 category-overrides rollback recompose, `configversion.go:515-525`) |
| T-25 residual | `support_recipients.go` / `support_tac_trust.go` | No |
| T-13 residual | README/enterprise "TLS Inspection" branding | No |

No re-verification lane was needed beyond this diff check — a stronger guarantee than a manual
re-read, since it rules out drift by construction rather than by sampling.
...
- **Current names / collision:**
  ...
  - PR-8's non-conforming assignment: `events.go:68` — `rb.rec.PolicyReason =
    mcperr.ReasonOf(err).Code()`, fired when durably committing the decision event itself fails
    (`"event_commit_failed"`, `"event_queue_saturated"`) — a durability-layer failure, not a policy
    decision, again overwriting whatever `d.Reason` the policy engine had just produced.
...
- **Recommended canonical name / fix:** stop overwriting `PolicyAction`/`PolicyReason` for the two
  pre-policy/post-policy gate cases; either leave them at whatever the policy engine actually produced (or
  ...
  `GateReason` or `HardFailReason`, for the redaction-failure and event-commit-failure cases). Land this
  before any PR-9 work builds a consumer (log pipeline, dashboard, or export mapping) on top of
  `PolicyReason`'s current, contract-violating values — the same "cheapest point to fix it" framing as
  T-32.
...
## Carried over, still open (re-confirmed this pass by diff-absence, see Wave 1 table)
...
| T-21 | "Config Version" names two unrelated, independently-incrementing counters | Open since 07-24. Unchanged; still compounded by T-32 (unescalated this pass). |
...
Terminology is **not** fully consistent. This narrow, one-day pass re-confirmed by diff-absence that all
eight previously-open findings are untouched, found the two feature areas newly shipped in this window
