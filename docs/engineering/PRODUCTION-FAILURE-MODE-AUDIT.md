...
| F-08 | DP unhealthy while UI healthy (CHAOS-09) | `/ready` gates only `session_secret`+`config_snapshot_validator` (`healthcheck.go:151-169`); `dpControlPlanePollFailing` + cert-expiry NOT gated → LB never ejects degraded DP. `HV` (agent) | N | N | N (/ready green) | OPEN | M | **MED** | High |
...
| F-10 | Scanner error mid-request (CHAOS-10/17) | ClamAV *error* fails **open** (`secscan.go:499-529`) vs timeout fails closed; plain-HTTP body read error fails **open + truncates** (`proxy_http.go:161-181`) vs inspect fails closed; **no `culvert_scan_errors_total`**, no `scan.on_error` config | N | partial | L | OPEN (2 paths) | A | MED | High |
...
| F-25 | DP cert renewal (CHAOS-12) | `renewDPCert` persists new cert/key but **never reloads the live gRPC client** (`dp_enrollment.go:362-408`); inert until `failover()` re-dials or restart; CP outage spanning the 30-day window bricks the DP at expiry. Contrast CDR path which hot-swaps (`cdr_health.go:254`). `HV` (agent) | N | N | L | CLOSED (brick) | M | **MED-HIGH** | High |
