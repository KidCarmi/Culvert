...
| F-12 | Config rollback with a failing disk (CHAOS-27) | **MITIGATED (2026-07-31):** root cause was one layer below the rollback — *nine of ten* enforcement-state stores destroyed their write error inside `Save()` (`_ = fileutil.AtomicWrite(...)`), so no caller could know. All such sites now go through `fileutil.AtomicWriteTracked` (`internal/fileutil/persist.go`): per-store failure accounting + reporter hook → WARN log, `config_persist_failed`/`_recovered` alerts (transition edge, via `deferStartupAlert`), report-only `/readyz` `config_persist_<store>` row, and `culvert_config_persist_{failing_stores,failures_total,failing}` metrics (`config_persist.go`). `applyConfigBackup` now returns the stores that failed inside its apply window, and the rollback API answers `500 {"status":"rolled_back_not_durable","applied":true,"persist_errors":[…]}` with a matching audit entry + SPA toast. Posture recorded: report, do not refuse. Remainder: `admin_settings.json`/`ui_users.json` (F-13), the audit writer (CHAOS-19), and the ST-10 residual writers | Y | Y `internal/fileutil/persist_test.go`, `config_persist_test.go` | Y (log+alert+/readyz+metrics) | **MITIGATED** | A (auto-detected recovery) + M (fix the FS) | MED | High |
