...
| F-31 | Response body outruns the scan window | Size-limit fail-open is **by design** (5 MiB default): the prefix is scanned, the remainder is forwarded. **MITIGATED:** the *signal* used to exist on one origin-chosen shape only — `logScanLimitExceeded` fired solely on a **declared** `Content-Length` pre-check, so a chunked plain-HTTP body and **every** over-limit SSL-inspect body (`scanInspectBody` has no pre-check) were forwarded partially-scanned with no counter, log or `scan_skipped` alert. Both truncation points now probe with `readScanPrefix` (limit+1) and signal on the forwarding path (`proxy_http.go`, `proxy_tunnel.go`, `security_scan.go`). Remainder: the fail-open posture itself and the `MaxDecompressBytes` decompression cap are still un-signalled | N (fail-open by design) | Y `proxy_scan_limit_signal_test.go` | Y (`stat_scan_skipped` + `scan_skipped` alert) | OPEN (alerted) | A | MED | High |
