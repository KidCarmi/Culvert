...
| F-11 | Auth IdP outage (CHAOS-16) | **FIXED (2026-08-02):** backend results are now three-valued (`authBackendOutcome`, `auth_health.go`). An ANSWER (allow/deny) keeps the configured CacheTTL; a FAILURE (dial/STARTTLS/service-bind/search error, transport error, non-200, unparseable body) is indeterminate — still denied fail-closed, but cached only for `authIndeterminateTTL` (5s, a load guard on a struggling directory) instead of 5m/2m, so an outage no longer outlives itself. LDAP gained the post-dial deadline (`conn.SetTimeout(ldapOpTimeout)`) that `DialWithDialer` never covered. Outages are now VISIBLE and distinct from credential failures: `culvert_auth_backend_unreachable_total{backend}` + `culvert_auth_backend_degraded{backend}`, an `auth_backend_reachability` contract row, and the `auth_backend_unreachable` alert. Remainder: no per-backend circuit breaker (bounded by the 5s gate) | Y | Y `auth_backend_outage_test.go` (T12 GREEN) | Y | **CLOSED** (bounded to 5s) | A | LOW | High |
| F-12 | Config rollback with a failing disk (CHAOS-27) | **FIXED (2026-07-31, CHAOS-45 run):** `applyConfigBackup` returns an error naming every file that failed to persist (collected via the new `fileutil` write-failure observer through a scoped collector); the handler answers `500` `status:"rolled_back_not_durable"` with `applied:true`/`durable:false`/`persist_errors`, and the audit entry records "NOT DURABLE". The in-memory apply stays unconditional by design. Test **T4 GREEN** (`configversion_rollback_durability_test.go`) | Y | Y | Y | **CLOSED** | M (fix disk + re-save) | MED | High |
| F-13 | Admin-settings save fails (CHAOS-33) | `go SaveAdminSettings()` fire-and-forget `admin_settings.go:437`; mutation API still returns 200 regardless — the **API contract gap is OPEN**. The *invisibility* half is MITIGATED (2026-07-31): the save routes through `fileutil.AtomicWrite`, so a failure is now counted (`culvert_storage_write_failures_total`), alerted (`storage_write_failed`), and surfaced as a `fail` row on `/api/diagnostics` naming `admin_settings.json` — the setting still vanishes on restart, but no longer in silence | partial | Y (observer) | Y | OPEN (API 200) | M | MED | High |
