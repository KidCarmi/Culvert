...
func TestPolicyStore_ReplaceAllStillResetsHitAccounting(t *testing.T) {
	ps := &PolicyStore{}
	ps.Add(PolicyRule{Priority: 1, Name: "reset", DestFQDN: "*", Action: ActionAllow})
	if match := ps.Evaluate("203.0.113.1", "", "unauth", "example.com", nil); match == nil {
		t.Fatal("rule did not match")
	}
	snapshot := ps.List()
	if snapshot[0].HitCount != 1 {
		t.Fatalf("precondition: HitCount = %d, want 1", snapshot[0].HitCount)
	}
	ps.ReplaceAll(snapshot)
	if got := ps.List()[0].HitCount; got != 0 {
		t.Fatalf("ReplaceAll retained HitCount = %d, want reset to 0", got)
	}
}
